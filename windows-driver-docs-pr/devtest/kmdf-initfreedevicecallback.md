...
description: InitFreeDeviceCallback 規則は、ドライバーが新しいフレームワークデバイスオブジェクトの初期化中にエラーを検出した場合や、ドライバーが \_ WdfControlDeviceInitAllocate の呼び出しから wdfdevice INIT 構造体を受け取った場合に、WdfDeviceInitFree を呼び出す必要があることを指定します。
