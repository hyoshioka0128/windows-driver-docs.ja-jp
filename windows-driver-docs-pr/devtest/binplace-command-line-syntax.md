...
実行可能ファイルをコピーする前に適切なシンボルがあることを確認する BinPlace をによりします。 このオプションを使用するには、SymChk ツールは、パスにする必要があります。 (SymChk は Windows のツールをデバッグ パッケージの一部です。 参照してください[Windows デバッグ](https://msdn.microsoft.com/library/windows/hardware/ff551063)詳細についてはします)。
...
BinPlace が .dbg ファイルを配置するを防ぎます。 場合、 **-j**スイッチを使用しても、これにより、BinPlace .dbg ファイルをポイントするバイナリを配置するからです。 このオプションを使用するには、SymChk ツールは、パスにする必要があります。 (SymChk は Windows のツールをデバッグ パッケージの一部です。 参照してください[Windows デバッグ](https://msdn.microsoft.com/library/windows/hardware/ff551063)詳細についてはします)。
