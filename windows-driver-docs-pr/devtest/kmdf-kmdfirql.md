---
title: KmdfIrql ルール (kmdf)
description: KmdfIrql ルールでは、ドライバーが、そのメソッドの最大の IRQL 以下である IRQL でフレームワーク メソッドを呼び出すことを指定します。
...
ms.locfileid: "67393788"
---
# <a name="kmdfirql-rule-kmdf"></a>KmdfIrql ルール (kmdf)


**KmdfIrql**ルールでは、ドライバーが、そのメソッドの最大の IRQL 小さい IRQL でフレームワーク メソッドを呼び出すことを指定します。

このルールと似ています、 [ **KmdfIrql2** ](kmdf-kmdfirql2.md)ルールですただし、、 **KmdfIrql2** IRQL 要件が、それに関連に依存するコールバック関数に対してルールが実行されます。オブジェクトの実行レベルの仕様。
...
<tbody>
<tr class="odd">
<td align="left"><p>実行<a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">Static Driver Verifier</a>を指定し、 <strong>KmdfIrql</strong>ルール。</p>
