---
title: Kmdファイヤ Ql ルール (kmdf)
description: Kmdno Ql ルールは、ドライバーが、そのメソッドの最大 IRQL 以下の IRQL でフレームワークメソッドを呼び出すことを指定します。
...
ms.locfileid: "72839486"
---
# <a name="kmdfirql-rule-kmdf"></a>Kmdファイヤ Ql ルール (kmdf)


**Kmdno ql**ルールは、ドライバーが、そのメソッドの最大 irql 以下の irql でフレームワークメソッドを呼び出すことを指定します。

このルールは、 [**KmdfIrql2**](kmdf-kmdfirql2.md)ルールに似ています。ただし、 **KmdfIrql2**規則は、関連するオブジェクトの実行レベルの指定に依存する IRQL 要件を持つコールバック関数で実行されます。
...
<tbody>
<tr class="odd">
<td align="left"><p><a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">静的なドライバーの検証ツール</a>を実行し、 <strong>Kmd起動 ql</strong>ルールを指定します。</p>
