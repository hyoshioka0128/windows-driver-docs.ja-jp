...
description: KmdfIrql2 規則は、ドライバーが、そのメソッドの最大 IRQL 以下の IRQL でフレームワークメソッドを呼び出すことを指定します。
...
ms.locfileid: "72839483"
---
# <a name="kmdfirql2-rule-kmdf"></a>KmdfIrql2 ルール (kmdf)


**KmdfIrql2**規則は、ドライバーが、そのメソッドの最大 irql 以下の irql でフレームワークメソッドを呼び出すことを指定します。

このルールは、 [**Kmdファイヤ ql**](kmdf-kmdfirql.md)ルールに似ています。ただし、 **Kmdfirql**ルールは、関連するオブジェクトの実行レベルの指定とは関係なく、IRQL の要件を持つコールバック関数で実行されます。
...
<tbody>
<tr class="odd">
<td align="left"><p><a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">静的ドライバー検証ツール</a>を実行し、 <strong>KmdfIrql2</strong>規則を指定します。</p>
