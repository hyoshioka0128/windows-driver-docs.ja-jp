---
title: KmdfIrql2 ルール (kmdf)
description: KmdfIrql2 ルールでは、ドライバーが、そのメソッドの最大の IRQL 以下である IRQL でフレームワーク メソッドを呼び出すことを指定します。
...
ms.locfileid: "67393785"
---
# <a name="kmdfirql2-rule-kmdf"></a>KmdfIrql2 ルール (kmdf)


**KmdfIrql2**ルールでは、ドライバーが、そのメソッドの最大の IRQL 小さい IRQL でフレームワーク メソッドを呼び出すことを指定します。

このルールと似ています、 [ **KmdfIrql** ](kmdf-kmdfirql.md)ルールですただし、、 **KmdfIrql** IRQL 要件が、それに関連の独立したコールバック関数に対してルールが実行されます。オブジェクトの実行レベルの仕様。
...
<tbody>
<tr class="odd">
<td align="left"><p>実行<a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">Static Driver Verifier</a>を指定し、 <strong>KmdfIrql2</strong>ルール。</p>
