...
ms.locfileid: "67392983"
---
# <a name="outputbufferapi-rule-kmdf"></a>OutputBufferAPI ルール (kmdf)


**OutputBufferAPI**ルールでは、バッファー取得のための正しい Ddi で使用されることを指定します、 [ *EvtIoWrite* ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfio/nc-wdfio-evt_wdf_io_queue_io_write)コールバック関数。 内で、 *EvtIoWrite*コールバック関数では、次の Ddi はバッファー取得のため呼び出すことができません。
...
<tbody>
<tr class="odd">
<td align="left"><p>実行<a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">Static Driver Verifier</a>を指定し、 <strong>OutputBufferAPI</strong>ルール。</p>
