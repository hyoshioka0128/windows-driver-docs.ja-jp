...
description: WdfRequestSendSyncAtDispatch2 ルールは、適切な IRQL 優先度レベル WdfRequestSend 関数が送信されることを確認します。
...
ms.locfileid: "67394010"
---
# <a name="wdfrequestsendsyncatdispatch2-rule-kmdf"></a>WdfRequestSendSyncAtDispatch2 ルール (kmdf)


**WdfRequestSendSyncAtDispatch2**ルールを確認、 [ **WdfRequestSend** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfrequest/nf-wdfrequest-wdfrequestsend)関数は、適切な IRQL の優先度レベルで送信します。

場合は、ドライバーの設定、WDF\_要求\_送信\_オプション\_同期フラグは、呼び出す必要があります、 [ **WdfRequestSend** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfrequest/nf-wdfrequest-wdfrequestsend) IRQL で関数パッシブを =\_レベル。 このフラグが設定されていない場合、ドライバーを呼び出す必要があります、 **WdfRequestSend** IRQL で関数のディスパッチを =\_レベル。

> [!NOTE]
>
> このルールのテストと同じ条件、 [ **WdfRequestSendSyncAtDispatch** ](wdfrequestsendsyncatdispatch.md)規則が異なるコード パスをテストするさまざまなハーネスを使用します。
...
<tbody>
<tr class="odd">
<td align="left"><p>実行<a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">Static Driver Verifier</a>を指定し、 <strong>WdfRequestSendSyncAtDispatch2</strong>ルール。</p>
