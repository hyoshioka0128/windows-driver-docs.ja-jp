...
次のコマンドは、MyTrace リアルタイム トレース セッションからのトレース メッセージを書式設定する、コマンド プロンプト ウィンドウに表示を調べる場合は後でテキスト ファイルに保存 Tracefmt を使用します。 コマンドの構文の詳細については、[Tracefmt](tracefmt.md)を参照してください。
