...
**IrqlKeWaitForMultipleObjects**ルーチンの呼び出し元は、次の場合を除き、IRQL &lt;= ディスパッチ\_レベルで実行できます。

-   *Timeout* &lt;&gt; 0 の場合、 [**KeWaitForMultipleObjects**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-kewaitformultipleobjects)ルーチンの呼び出し元は、IRQL &lt;= APC\_レベルで実行されている必要があります。
-   *Timeout* ! = NULL および \**timeout* = 0 の場合、 [**KeWaitForMultipleObjects**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-kewaitformultipleobjects)ルーチンの呼び出し元は、IRQL = DISPATCH\_レベルで実行されている必要があります。

-   *Timeout* = **NULL**または \**timeout* ! = 0 の場合、 [**KEWAITFORMULTIPLEOBJECTS**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-kewaitformultipleobjects)ルーチンの呼び出し元は、IRQL &lt;= APC\_レベルで実行されている必要があります。
