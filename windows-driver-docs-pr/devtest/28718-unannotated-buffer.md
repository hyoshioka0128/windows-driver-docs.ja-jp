...
この警告は、関数に渡されるか、関数によって返されるバッファーにソース コード注釈言語 (SAL) の注釈があるない場合に報告されます。 静的分析ツールでは、バッファー オーバーランを検出するために、このような注釈を使用できます。 注釈を追加する方法の詳細については、[C と C++ コードの欠陥を削減する SAL 注釈を使って](https://go.microsoft.com/fwlink/p/?linkid=247283)と**に注釈を付ける関数のパラメーターと戻り値**を参照してください。
