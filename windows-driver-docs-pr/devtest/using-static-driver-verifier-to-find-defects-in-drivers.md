...
1.  **関数ロール型を使用してドライバーで提供される関数を宣言する**
    ...
    サポートされている各ドライバーモデルには、ドライバーのコールバック関数とディスパッチルーチンに対する一連の関数ロールの種類があります。 これらの関数のロール型は、WDK ヘッダーファイルで宣言されています。 たとえば、次に示すのは、 \_ Wdm に表示される DRIVER INITIALIZE ロールの種類の関数プロトタイプです。
