---
title: 初期化\_RegisterInterrupt rule (ndis)
description: Init\_RegisterInterrupt 規則は、初期化プロセスで何らかの問題が発生した場合、またはミニポートドライバーの停止時に、通常は初期化中に発生する割り込みの登録を元に戻す必要があることを指定します。NdisMRegisterInterruptEx が MiniportInitializeEx 中に少なくとも1回呼び出される場合は、NdisMDeregisterInterruptEx 関数を MiniportHaltEx で少なくとも1回呼び出す必要があります。
...
ms.locfileid: "72839399"
---
# <a name="init_registerinterrupt-rule-ndis"></a>初期化\_RegisterInterrupt rule (ndis)


Init\_RegisterInterrupt 規則は、初期化プロセスで何らかの問題が発生した場合、またはミニポートドライバーの停止時に、通常は初期化中に発生する割り込みの登録を元に戻す必要があることを指定します。
