---
title: Usbkmdファイヤ Ql ルール (kmdf)
description: Usbkmdql Ql ルールは、KMDF ドライバーが、正しくない IRQL レベルで USB 固有のデバイスドライバーインターフェイス (DDI) を呼び出さないことを指定します。
ms.assetid: 6706db3e-cd27-4d0b-ac91-dfe2d6444e7f
ms.date: 05/21/2018
keywords:
- Usbkmdファイヤ Ql ルール (kmdf)
topic_type:
- apiref
api_name:
- UsbKmdfIrql
...
ms.locfileid: "72839433"
---
# <a name="usbkmdfirql-rule-kmdf"></a>Usbkmdファイヤ Ql ルール (kmdf)


**Usbkmdql ql**ルールは、kmdf ドライバーが、正しくない IRQL レベルで USB 固有のデバイスドライバーインターフェイス (DDI) を呼び出さないことを指定します。
...
<tbody>
<tr class="odd">
<td align="left"><p><a href="https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier" data-raw-source="[Static Driver Verifier](https://docs.microsoft.com/windows-hardware/drivers/devtest/static-driver-verifier)">静的ドライバー検証ツール</a>を実行し、 <strong>Usbkmd起動 ql</strong>ルールを指定します。</p>
