---
title: すべてのドライバー開発者のための概念
description: このセクションでは、すべてのドライバー開発者に必要な概念について説明します。
ms.assetid: 639bba8d-d0de-401b-aeb4-c9ad43760e54
keywords:
- ドライバーの基本事項
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 42ec2df44588dff7f2a37f70f861720c46b5e267
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/31/2019
ms.locfileid: "56518335"
---
# <a name="concepts-for-all-driver-developers"></a>すべてのドライバー開発者のための概念


このセクションの内容

-   [ユーザー モードとカーネル モード](user-mode-and-kernel-mode.md)
-   [仮想アドレス領域](virtual-address-spaces.md)
-   [デバイス ノードとデバイス スタック](device-nodes-and-device-stacks.md)
-   [I/O 要求パケット](i-o-request-packets.md)
-   [ドライバー スタック](driver-stacks.md)
-   [ミニドライバーとドライバーのペア](minidrivers-and-driver-pairs.md)
-   [Windows Driver Kit (WDK) のヘッダー ファイル](header-files-in-the-windows-driver-kit.md)
-   [異なるバージョンの Windows に対するドライバーの作成](platforms-and-driver-versions.md)

 

 





