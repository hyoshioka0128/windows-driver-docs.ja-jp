...
Windows 8.1、デバイスの製造元は、ユーザーが PC に自分のデバイスを接続するときに自動的にインストールする UWP デバイス アプリを構成できます。 このトピックでは、自動インストールの動作とどのようにアプリ、メタデータ、およびドライバー更新、アンインストールについて説明します。 デバイスのアプリに関する詳細については、次を参照してください。[満たす UWP デバイス アプリ](meet-uwp-device-apps.md)します。
