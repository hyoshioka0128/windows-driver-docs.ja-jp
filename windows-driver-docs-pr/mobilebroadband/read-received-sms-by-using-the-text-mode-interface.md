...
**注**   SMS クライアント アプリは、長期にわたる複数のセグメントがメッセージし、メッセージ全体を再構築は、連結する Windows によって提供されるデコードされたセグメント化情報を使用できます。 セグメント化された SMS メッセージの詳細については、[Windows が自動的に時間の長いメッセージをセグメント](windows-automatically-segments-long-messages.md)を参照してください。
