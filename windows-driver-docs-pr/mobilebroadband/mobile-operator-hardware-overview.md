...
**注**   モバイルブロードバンドクラスドライバーでは、USSD、EAP、および複数の PDP コンテキストがサポートされていますが、デスクトップのエディション (Home、Pro、Enterprise、および教育) のオプションコンポーネントとして、windows 8、Windows 8.1、または windows 10 がサポートされています。ハードウェア認定の要件。 ただし、ハードウェア認定を行うには、Windows 10 Mobile に複数の PDP コンテキストが必要です。
...
-   オペレーターによって事前にプロビジョニングされた場合、またはそのシステムの SIM を使用して正常に接続される**前**に OTA によってプロビジョニングされた場合、デバイスは、Contexttype が**インターネット**に設定された最初のプロビジョニング済みコンテキストとして、インターネット PDP コンテキストを提供します。MBIM セクション10.5.13.5 で定義されているとおりに Windows によって照会されます。 これにより、接続時に接続ロジックでこの APN 情報が使用されるようになります。
