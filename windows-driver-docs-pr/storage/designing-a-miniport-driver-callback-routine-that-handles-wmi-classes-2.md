...
このセクションでは、WMI メソッドが含まれている例の WMI クラスを使用し、対応するミニポート ドライバー コールバック ルーチンがどのようにについて説明します。 WMI メソッドを実行するミニポート ドライバー コールバック ルーチンの詳細については、次を参照してください。 [ **HwScsiWmiExecuteMethod**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/scsiwmi/nc-scsiwmi-pscsiwmi_execute_method)します。
