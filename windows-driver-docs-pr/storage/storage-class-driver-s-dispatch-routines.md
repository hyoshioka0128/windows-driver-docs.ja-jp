---
title: 記憶域クラス ドライバーのディスパッチ ルーチン
description: 記憶域クラス ドライバーのディスパッチ ルーチン
...
ms.locfileid: "56552870"
---
# <a name="storage-class-drivers-dispatch-routines"></a>記憶域クラス ドライバーのディスパッチ ルーチン
...
ストレージ クラス ドライバーが必要、 [ **DispatchPnP** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdm/nc-wdm-driver_dispatch)日常的なを開始する要求の停止、およびデバイスを削除要求または応答を他の PnP デバイスがページング パス上の通知などです。 PnP 開始要求の処理について詳しくは、[、記憶域クラス ドライバーの PnP 開始を処理](handling-pnp-start-in-a-storage-class-driver.md)を参照してください。 PnP 他の要求の処理について詳しくは、[記憶域周辺機器への PnP の要求を処理](handling-pnp-requests-to-storage-peripherals.md)を参照してください。

ストレージ クラス ドライバーが必要、 [ **DispatchPower** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdm/nc-wdm-driver_dispatch)ルーチンのデバイスの電源状態を設定するための要求。 詳細については、[記憶域周辺機器に電力の要求を処理](handling-power-requests-to-storage-peripherals.md)を参照してください。
