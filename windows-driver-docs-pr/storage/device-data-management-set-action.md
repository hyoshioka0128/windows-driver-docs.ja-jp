...
管理アクションがで指定された、 [**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)のシステムのバッファーに含まれている構造体、[ **IOCTL\_ストレージ\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ni-ntddstor-ioctl_storage_manage_data_set_attributes)要求。
...
このアクションの場合、 **ParameterBlockOffset**と**ParameterBlockLength**のメンバー、 [**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)構造体は、通知操作のパラメーターを指定します。 これらのパラメーターとして書式設定、 [**デバイス\_DSM\_通知\_パラメーター** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_notification_parameters)構造体。
...
このアクションの場合、 **ParameterBlockOffset**と**ParameterBlockLength**のメンバー、 [**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)構造体は、オフロードのパラメーターの読み取り操作を指定します。 これらのパラメーターとして書式設定、 [**デバイス\_DSM\_オフロード\_読み取り\_パラメーター** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_offload_read_parameters)構造体。
...
このアクションの場合、 **ParameterBlockOffset**と**ParameterBlockLength**のメンバー、 [**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)構造体は、オフロードのパラメーターの書き込み操作を指定します。 これらのパラメーターとして書式設定、 [**デバイス\_DSM\_オフロード\_書き込み\_パラメーター** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_offload_write_parameters)構造体。
...
1 つの論理ブロックの範囲が指定された[**デバイス\_データ\_設定\_範囲**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)構造体。
...
1 つの論理ブロックの範囲が指定された[**デバイス\_データ\_設定\_範囲**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)構造体。 修復のコピーの数がで指定された、**デバイス\_データ\_設定\_修復\_パラメーター**構造体で指定された入力バッファーにある、 **ParameterBlockOffset**のメンバー、 [**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)構造体。
...
<span id="DeviceDsmAction_DrtQuery"></span><span id="devicedsmaction_drtquery"></span><span id="DEVICEDSMACTION_DRTQUERY"></span>**DeviceDsmAction\_DrtQuery** (8 |DeviceDsmActionFlag\_NonDestructive) ドライバーは、追跡、記憶域スペースのダーティ範囲に含まれている場合に、指定された論理ブロックの範囲とチェックのクエリが実行されます。 範囲の同期状態が返された、 **OperationStatus**のメンバー、 [**デバイス\_管理\_データ\_設定\_属性\_出力**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes_output)構造体。

入力、1 つの論理ブロックの範囲が指定されて[**デバイス\_データ\_設定\_範囲**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)構造体。
...
新しいターゲットの優先順位が設定されて、**デバイス\_DSM\_NVCACHE\_変更\_優先度\_パラメーター**構造にある**ParameterBlockOffset**で[**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)します。 1 つまたは複数の優先順位を変更する論理ブロック範囲が与えられます[**デバイス\_データ\_設定\_範囲**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)構造体。
...
<span id="DeviceDsmAction_NvCache_Evict"></span><span id="devicedsmaction_nvcache_evict"></span><span id="DEVICEDSMACTION_NVCACHE_EVICT"></span>**DeviceDsmAction\_NvCache\_削除**(15 |DeviceDsmActionFlag\_NonDestructive) ドライバーはキャッシュの中からデータを削除します。 すべてのデータを削除するには、デバイスを設定\_DSM\_フラグ\_全体\_データ\_設定\_範囲フラグ、**フラグ**のメンバー [ **デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)は含まれませんと[**デバイス\_データ\_設定\_範囲**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)構造体。

削除する特定の論理ブロックの範囲が 1 つまたは複数指定された[**デバイス\_データ\_設定\_範囲**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)構造体。
...
ビットごとの場合**または**の**デバイス\_データ\_管理\_設定\_アクション**定数値、および**DeviceDsmActionFlag\_NonDestructive**フラグが実行されるため、指定の操作は非破壊的な。 ドライバーを安全に転送できるこのフラグが設定されている場合、 [ **IOCTL\_ストレージ\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ni-ntddstor-ioctl_storage_manage_data_set_attributes)要求いても、スタックで [次へ] の下位のドライバーをドライバーは、指定されたアクションを処理しません。

&gt; \[!注\]&gt;を転送する前に、 [ **IOCTL\_ストレージ\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ni-ntddstor-ioctl_storage_manage_data_set_attributes)要求と、ドライバーで指定されているデータ セットの範囲のブロックの通常の処理を実行します、 [**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)構造体。 このブロックは**DataSetRangesOffset**として書式設定された 1 つ以上の連続したエントリで構成される[**デバイス\_データ\_設定\_範囲**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_data_set_range)構造体。 データ セットの範囲の長さ、(バイト単位) を設定、 **DataSetRangesLength**のメンバー**デバイス\_管理\_データ\_設定\_属性**します。
...
[**デバイス\_DSM\_通知\_パラメーター**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_dsm_notification_parameters)

[**デバイス\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ns-ntddstor-_device_manage_data_set_attributes)

[**IOCTL\_ストレージ\_管理\_データ\_設定\_属性**](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntddstor/ni-ntddstor-ioctl_storage_manage_data_set_attributes)
