...
以降、ルートで、デバイス全体のツリーを表示するには使用 **! devnode 0 1**:
...
次の例には、PCI バスによって作成されたデバイスが表示されます。 このデバイスには、DNF はありません。\_作り上げられたフラグを設定します。
...
Devnode いない CM リソースの一覧が開始されていないとが要求されたリソースが、リソースの使用されていないために注意してください。

2. 従来、ドライバーのこの devnode で格納されているリソースがないことに注意してください。
...
次の種類のデバイス ドライバーのデバイス オブジェクトの一覧を取得できます。
...
最後に、デバイス オブジェクトによって参照される devnode をダンプすることができます。 この devnode は、デバイス ツリーにはリンクされません。 これは、"擬似-devnode"レガシ デバイスのリソースを要求するために使用を表します。 メモ、DNF\_リソース\_報告を示すフラグをデバイスが報告された、検出されたデバイス。
