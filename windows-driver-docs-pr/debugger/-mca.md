---
title: mca
description: X86 mca 拡張機能の対象のコンピュータはマシン チェック アーキテクチャ (MCA) のレジスタが表示されます。 Itanium のターゲット コンピューター、mca 拡張子 MCA エラー レコードが表示されます。
ms.assetid: 452bfbf2-fcab-4a71-bfd0-b02afe30df74
keywords:
- マシン チェック アーキテクチャ (MCA)
...
## <a name="span-idddkmcadbgspanspan-idddkmcadbgspanparameters"></a><span id="ddk__mca_dbg"></span><span id="DDK__MCA_DBG"></span>パラメーター


<span id="_______Address______"></span><span id="_______address______"></span><span id="_______ADDRESS______"></span> *アドレス*   
(Itanium のターゲットのみ)MCA エラー レコードのアドレスを指定します。

<span id="_______Flags______"></span><span id="_______flags______"></span><span id="_______FLAGS______"></span> *フラグ*   
(Itanium のターゲットのみ)出力のレベルを指定します。 *フラグ*次のビットの組み合わせにすることができます。 既定値は、0 xff は、ログに存在するすべてのセクションが表示されます。
...
この拡張機能コマンドは、x86 ベースまたは Itanium にのみ使用できる対象のコンピュータ。

<a name="remarks"></a>注釈
-------

Itanium のターゲットで **! mca**システムの抽象化レイヤー (SAL) の MCA エラー レコードを表示します。 この拡張機能からの出力の例を次に示します。
...
X86 ターゲット、 **! mca** active プロセッサでサポートされているマシン チェック登録表示します。 CPU の基本的な情報も表示されます (によって表示される同じ[ **! cpuinfo**](-cpuinfo.md))。 この拡張機能からの出力の例を次に示します。
