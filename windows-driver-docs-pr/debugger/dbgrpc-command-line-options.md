...
リモート コンピューターから情報を表示させるために使用できます。 スラッシュ記号は、サーバー名にする必要があります前ありません。 リモートでさせるための使用に関する詳細については、[させるためのツールを使用して](using-the-dbgrpc-tool.md)を参照してください。
...
使用するリモートのトランスポートを指定します。 可能な値*ProtSeq*は**ncacn\_ip\_tcp** (TCP プロトコル) および**ncacn\_np** (名前付きパイプ プロトコル)。 TCP プロトコルをお勧めします。 リモートでさせるための使用に関する詳細については、[させるためのツールを使用して](using-the-dbgrpc-tool.md)を参照してください。
...
指定したセルの RPC 状態情報を表示します。 例については、[RPC セル情報の取得](get-rpc-cell-information.md)を参照してください。
...
エンドポイントは、システムの RPC 状態情報を検索します。 例については、[RPC エンドポイント情報の取得](get-rpc-endpoint-information.md)を参照してください。
...
スレッドは、システムの RPC 状態情報を検索します。 例については、[RPC スレッド情報の取得](get-rpc-thread-information.md)を参照してください。
...
検索サーバー側のシステムの RPC 状態情報は、(SCALL) 情報を呼び出します。 例については、[RPC 呼び出し情報の取得](get-rpc-call-information.md)を参照してください。

<span id="_______-a______"></span><span id="_______-A______"></span> **-**   
検索システムの RPC 状態情報がクライアントでは、(CCALL) 情報を呼び出します。 例については、[RPC クライアント呼び出し情報の取得](get-rpc-client-call-information.md)を参照してください。 このオプションでは、完全な RPC 状態情報が必要です。
