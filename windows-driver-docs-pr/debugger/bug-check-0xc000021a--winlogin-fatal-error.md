...
ms.locfileid: "71025523"
---
# <a name="bug-check-0xc000021a-winlogon_fatal_error"></a>バグ チェック 0xC000021A:WINLOGON\_の\_致命的なエラー

WINLOGON\_の致命的\_なエラーのバグチェックでは、値0xc000021a が指定されています。 これは、Winlogon プロセスが予期せず終了したことを意味します。

> [!IMPORTANT]
> このトピックはプログラマーを対象としています。 コンピューターの使用中にブルースクリーンのエラーコードが表示された顧客の場合は、「[ブルースクリーンエラーのトラブルシューティング](https://www.windows.com/stopcode)」を参照してください。

## <a name="winlogon_fatal_error-parameters"></a>WINLOGON\_の\_致命的なエラーパラメーター
...
システムファイルが一致しないと、このエラーが発生することもあります。 これは、バックアップからハードディスクを復元した場合に発生する可能性があります。 一部のバックアッププログラムでは、使用中のシステムファイルの復元をスキップする場合があります。
...
**ユーザーモードのデバイスドライバー、システムサービス、またはサードパーティ製のアプリケーションでエラーを解決するには、次のようにします。** バグチェック0xC000021A はユーザーモードプロセスで発生するため、最も一般的な発生はサードパーティ製のアプリケーションです。 新しいデバイスドライバー、システムサービス、またはサードパーティ製のアプリケーションをインストールした後にエラーが発生した場合は、新しいソフトウェアを削除するか無効にして、原因を特定する必要があります。 考えられる更新プログラムについて、ソフトウェアの製造元にお問い合わせください。

**システムファイルが一致しない問題の解決:** 最近バックアップからハードディスクを復元した場合は、更新されたバージョンのバックアップ/復元プログラムが製造元から入手可能かどうかを確認してください。

これらの手順は、追加情報の収集に役立つ場合があります。
...
トラブルシューティングのために要素を分離するにはセーフモードを使用し、必要に応じて Windows を使用することを検討してください。 セーフモードを使用すると、Windows の起動時に最小限必要なドライバーとシステムサービスのみが読み込まれます。 セーフモードに移行するには、設定 で **更新とセキュリティ** を使用します。 [**回復**-の&gt;**詳細スタートアップ**] を選択して、メンテナンスモードで起動します。 表示されたメニューで **、[** - **詳細設定オプション** - &gt; ] [&gt; **スタートアップ設定** -の**再起動**] の順に選択&gt;します。 Windows が**起動設定**画面に再起動したら、オプション、4、5、または6を選択してセーフモードで起動します。
