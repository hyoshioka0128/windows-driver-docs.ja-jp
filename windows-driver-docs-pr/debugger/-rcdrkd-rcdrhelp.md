---
title: rcdrkd.rcdrhelp
description: Rcdrkd.rcdrhelp コマンドでは、RCDRKD デバッガーの拡張機能コマンドのヘルプを表示します。
ms.assetid: E4BCAFB5-BC4F-4C7D-B011-67F86E1C9E99
keywords:
- デバッグ rcdrkd.rcdrhelp Windows
ms.date: 05/23/2017
topic_type:
- apiref
api_name:
- rcdrkd.rcdrhelp
api_type:
- NA
ms.localizationpriority: medium
ms.openlocfilehash: 678d245ba0080c913873bdd274bb57fb6af6f8e9
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/31/2019
ms.locfileid: "56571326"
---
# <a name="rcdrkdrcdrhelp"></a>!rcdrkd.rcdrhelp


**! Rcdrkd.rcdrhelp**コマンド RCDRKD デバッガーの拡張機能コマンドのヘルプを表示します。

## <a name="span-iddllspanspan-iddllspandll"></a><span id="DLL"></span><span id="dll"></span>DLL


Rcdrkd.dll

## <a name="span-idseealsospansee-also"></a><span id="see_also"></span>参照してください。


[RCDRKD 拡張機能](rcdrkd-extensions.md)

 

 






