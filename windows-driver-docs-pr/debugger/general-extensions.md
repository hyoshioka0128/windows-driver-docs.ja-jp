...
デバッガーでは、これらの拡張機能コマンドの適切なバージョンが自動的に読み込まれます。 別のバージョンを手動で読み込んだ場合を除き、使用されている DLL のバージョンを追跡する必要はありません。 既定のモジュールの検索順序の詳細については、次を参照してください。[デバッガー拡張機能コマンドの使用](using-debugger-extension-commands.md)します。 拡張機能モジュールを読み込む方法の詳細については、次を参照してください。[デバッガー拡張 Dll の読み込み](loading-debugger-extension-dlls.md)します。
