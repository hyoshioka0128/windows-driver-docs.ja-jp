...
このコマンドは、CTRL キーを押しながら O キーを押すか、[**ソースファイルを開く] (ctrl + o)** ボタン ([ソースファイルを開く] ボタンのスクリーンショット![](images/tbopen.png)) をクリックした場合と同じです。
...
2.  **[ファイルの種類]** ボックスの一覧で、開くファイルの種類を選択します。 **[ソースファイルを開く]** ダイアログボックスには、選択した拡張子を持つファイルのみが表示されます。
    また、 **[ファイル名]** ボックスのワイルドカードパターンを使用して、特定の拡張子を持つファイルのみを**表示  こと**もできます。 新しいワイルドカードパターンは、変更するまでセッションに保持されます。 ワイルドカードパターンの任意の組み合わせをセミコロンで区切って使用できます。 たとえば、「\*」と入力**します。◇\*。始め\*。CPP**では、これらの拡張子を持つすべてのファイルが表示されます。 行の最大文字数は251文字です。
