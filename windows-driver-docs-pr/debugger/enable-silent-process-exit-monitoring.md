---
title: サイレント プロセス終了の監視を有効にします。
description: サイレント プロセス終了の監視を有効にします。
ms.assetid: 3FEA9660-541B-42D0-B020-CECEBF14BB4D
ms.date: 11/28/2017
ms.localizationpriority: medium
ms.openlocfilehash: da0530cf8c63ace48b68f03478af099b7fc54b10
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/31/2019
ms.locfileid: "56531701"
---
# <a name="enable-silent-process-exit-monitoring"></a>サイレント プロセス終了の監視を有効にします。


## <span id="ddk_enable_pool_tagging_dtools"></span><span id="DDK_ENABLE_POOL_TAGGING_DTOOLS"></span>


**サイレント プロセス終了の監視を有効にする**フラグ サイレント終了は、プロセスの監視を有効にします。

<table>
<colgroup>
<col width="50%" />
<col width="50%" />
</colgroup>
<tbody>
<tr class="odd">
<td align="left"><p><strong>16 進数の値</strong></p></td>
<td align="left"><p>0x200</p></td>
</tr>
<tr class="even">
<td align="left"><p><strong>シンボリック名</strong></p></td>
<td align="left"><p>FLG_MONITOR_SILENT_PROCESS_EXIT</p></td>
</tr>
<tr class="odd">
<td align="left"><p><strong>変換先</strong></p></td>
<td align="left"><p>イメージ ファイルのレジストリ エントリ</p></td>
</tr>
</tbody>
</table>

 

### <a name="span-idcommentsspanspan-idcommentsspancomments"></a><span id="comments"></span><span id="COMMENTS"></span>コメント

サイレント終了のプロセスを監視する方法の詳細については、[サイレント プロセス終了の監視](registry-entries-for-silent-process-exit.md)を参照してください。

 

 





