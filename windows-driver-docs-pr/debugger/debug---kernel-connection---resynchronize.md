---
title: 接続の再同期をカーネルをデバッグします。
description: 接続の再同期をカーネルをデバッグします。
ms.assetid: b388dd9e-8d5c-4e29-8e81-a2be7c95437c
keywords:
- 接続の再同期をカーネルをデバッグします。
- カーネルの接続の再同期をデバッグ対象のコンピューターとの同期
ms.date: 05/23/2017
...
をポイント**カーネル接続**順にクリックします**再同期**上、**デバッグ**カーネル デバッグ ターゲットとの接続を再確立しようとするデバッガー メニューコンピューター。

このコマンドは、CTRL + ALT + R キーを押すと同じです。 (することができますも押して CTRL + R KD で)。

ターゲットが応答していない場合は、このコマンドを使用します。
