...
このバージョンでは、タイム トラベルのトレースを追加します。 時間、出張のデバッグ、プロセスを記録し、後で両方 forwards と backwards 再生することができます。 タイム トラベルのデバッグ (TTD) を使用することができるため、バグが見つかるまで、問題を再現するのではなく、デバッガー セッションを「巻き戻す」問題を簡単をデバッグできます。 詳細については、[タイム トラベルのデバッグ - 概要](time-travel-debugging-overview.md)を参照してください。
