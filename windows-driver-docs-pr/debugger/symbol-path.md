...
@No__t_0[コマンドラインオプション](command-line-options.md)を使用して、CDB および KD でのレイジーシンボルの読み込みをオフにすることができます。 シンボルの読み込みを強制するには、`ld` [ **(読み込みシンボル)** ](ld--load-symbols-.md)コマンドを使用するか、`/f` オプションと共に `.reload` [ **(モジュールの再読み込み)** ](-reload--reload-module-.md)コマンドを使用することもできます。
...
-   [@No__t_1](-sympath--set-symbol-path-.md)コマンドを使用して、パスの表示、設定、変更、または追加を行います。 @No__t_0 [ **(シンボルストアパスの設定)** ](-symfix--set-symbol-store-path-.md)コマンドは `.sympath` に似ていますが、いくつかの入力が保存されます。
...
@No__t_0[コマンドラインオプション](command-line-options.md)を使用した場合、デバッガーはシンボルパス環境変数を無視します。
