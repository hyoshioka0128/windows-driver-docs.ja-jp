...
呼び出しを使用して、バス ドライバー スタックとすべての通信が実行される、 [ **CreateFile**](https://msdn.microsoft.com/library/windows/desktop/aa363858)、 **ReadFile**、 **WriteFile**、および**DeviceIoControl**関数で、Microsoft Windows SDK のドキュメントに記載されています。 バス ドライバー スタックとの通信の詳細については、[静止画像デバイスへのアクセスのカーネル モードのドライバー](accessing-kernel-mode-drivers-for-still-image-devices.md)を参照してください。
