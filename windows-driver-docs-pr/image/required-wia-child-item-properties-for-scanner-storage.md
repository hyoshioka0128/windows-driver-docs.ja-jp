---
title: スキャナー記憶域の必須 WIA 子項目のプロパティ
description: スキャナー記憶域の必須 WIA 子項目のプロパティ
ms.assetid: 47640b56-d6d9-4ad6-b973-be9fd8992a2c
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 99c76d41b3525ead0f1f616068b9074e1a4ad101
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 04/23/2019
ms.locfileid: "63375848"
---
# <a name="required-wia-child-item-properties-for-scanner-storage"></a>スキャナー記憶域の必須 WIA 子項目のプロパティ


WIA スキャナー ストレージ (ファイル) の子項目が次の WIA プロパティをサポートするために必要です。

[**WIA\_IPA\_アクセス\_権限**](https://msdn.microsoft.com/library/windows/hardware/ff551518)

[**WIA\_IPA\_項目\_カテゴリ**](https://msdn.microsoft.com/library/windows/hardware/ff551581)

[**WIA\_IPA\_完全\_項目\_名**](https://msdn.microsoft.com/library/windows/hardware/ff551561)

[**WIA\_IPA\_項目\_名**](https://msdn.microsoft.com/library/windows/hardware/ff551590)

 

 




