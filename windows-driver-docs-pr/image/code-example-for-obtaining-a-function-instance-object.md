---
title: 関数のインスタンス オブジェクトの取得のコード例
description: 関数のインスタンス オブジェクトの取得のコード例
...
ms.locfileid: "63373210"
---
# <a name="code-example-for-obtaining-a-function-instance-object"></a>関数のインスタンス オブジェクトの取得のコード例


次のコード例には、関連する関数インスタンスの現在のオブジェクトを取得する 2 つのクラス メンバーを含むサンプル クラス (CWSDDevice) の宣言が含まれています。

-   CWSDDevice::m\_pFunctionDiscovery

-   CWSDDevice::m\_pFunctionInstance

コード例では、これらのメンバーを初期化するためにメソッドと現在の関数インスタンスのプロパティ ストアからデバイスのプロパティを読み取る方法も示します。 **CWSDDevice::InitializeConnection**メソッドに記載されている手順を示しています[関数インスタンスのオブジェクトを取得する](obtaining-a-function-instance-object.md)を表す現在の関数インスタンス オブジェクトを取得するには。現在 web サービス スキャナー デバイス インスタンス。
