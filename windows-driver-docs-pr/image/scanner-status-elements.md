---
title: スキャナーの状態要素
description: スキャナーの状態要素
ms.assetid: 9c616805-a25b-4fd3-bc84-4bf40b58cab6
ms.date: 11/28/2017
ms.localizationpriority: medium
ms.openlocfilehash: 630ca755ac141b86d41cb90da4e81a4307f26b30
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 04/23/2019
ms.locfileid: "63370302"
---
# <a name="scanner-status-elements"></a>スキャナーの状態要素


このセクションについて説明します、 **ScannerStatus**要素とその子。 **ScannerStatus** WSD スキャン サービス スキーマのセクションはオートマトン (時刻や、スキャナーでの状態の変化) を制御する情報を格納します。

このセクションの内容:

[**ActiveConditions**](activeconditions.md)

[**ClearTime**](cleartime.md)

[**コンポーネント**](component.md)

[**ConditionHistory**](conditionhistory.md)

[**ConditionHistoryEntry**](conditionhistoryentry.md)

[**DeviceCondition**](devicecondition.md)

[**DeviceCondition と ConditionHistoryEntry の name 要素**](name-element-for-devicecondition-and-conditionhistoryentry.md)

[**ScannerCurrentTime**](scannercurrenttime.md)

[**ScannerState**](scannerstate.md)

[**ScannerStateReason**](scannerstatereason.md)

[**ScannerStateReasons**](scannerstatereasons.md)

[**ScannerStatus**](scannerstatus.md)

[**重要度**](severity.md)

[**時間**](time.md)

 

 





