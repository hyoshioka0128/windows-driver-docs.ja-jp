---
title: WSD スキャン サービス操作の要素
description: WSD スキャン サービス操作の要素
...
ms.locfileid: "56559025"
---
# <a name="wsd-scan-service-operation-elements"></a>WSD スキャン サービス操作の要素


このセクションでは、操作の XML 要素と、WSD スキャン サービス スキーマを定義する操作に固有の子要素について説明します。 クライアントが送信を*Xxx * * * 要求** WSD スキャン サービスに操作の要素の場合、対応する WSD スキャン サービスが応答*Xxx * * * 応答** 操作の要素または適切なエラー コード。

WSDL サービスの説明内で操作の一覧を拡張することはできません。 非標準の操作を実装する場合は、する必要がある一部の新しい**portType**を拡張する型、WSD スキャン サービス定義にします。

WSD スキャン サービス スキーマでは、次の操作の要素と操作に固有の子要素を定義します。
...
[**ElementData JobElements 要素**](elementdata-for-jobelements-element.md)

[**ElementData ScannerElements 要素**](elementdata-for-scannerelements-element.md)
