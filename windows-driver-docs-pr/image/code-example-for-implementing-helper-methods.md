---
title: ヘルパー メソッドを実装するためのコード例
description: ヘルパー メソッドを実装するためのコード例
...
ms.locfileid: "56570101"
---
# <a name="code-example-for-implementing-helper-methods"></a>ヘルパー メソッドを実装するためのコード例

> [!IMPORTANT]  
> WSD 見据えて機能は非推奨とされているし、WSD の挑戦者に関連するすべてのドキュメントは、2018 年に削除されます。

次のコード例では、WSD チャレンジ インターフェイスを初期化し、デバイスのチャレンジの登録に使用されるヘルパー メソッドの実装を示します。
