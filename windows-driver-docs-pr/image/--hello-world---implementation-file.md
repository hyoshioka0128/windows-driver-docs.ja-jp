---
title: Hello World' 実装ファイル
description: Hello World' 実装ファイル
...
ms.locfileid: "56551273"
---
# <a name="hello-world-implementation-file"></a>実装ファイルの 'hello World'

この WIA ミニドライバーは、2 つの関数をエクスポートする単純な DLL (を参照してください[定義ファイルの"Hello World"](--hello-world---definition-file.md)) 3 つの COM インターフェイスを実装して (を参照してください[COM インターフェイスを提供する](providing-a-com-interface.md))。 WIA ミニドライバーの次のコード例は、作業用のドライバーにコンパイルできます。 この WIA ミニドライバーを作成する項目のツリーを持ち、ルート項目は、子項目はありませんし、データを転送することはできません。

*Hellowld.cpp*ファイルは、次を含める必要があります。
