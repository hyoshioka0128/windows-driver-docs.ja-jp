...
4.  デバイスがインボックスではない場合は、 **[Driver Package]** (ドライバー パッケージ)、 **[Driver Locales]** (ドライバー ロケール)、 **[Symbols (optional)]** (シンボル (省略可能)) に、必要な情報を入力します。 **注**:ドライバー パッケージの相対パスとファイル名の合計は、160 文字未満にする必要があります。 これを超えた場合、送信処理が失敗します。
