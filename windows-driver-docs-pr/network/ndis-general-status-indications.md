---
title: NDIS 一般状態表示
description: NDIS 一般状態表示
ms.assetid: 0ce19114-9861-4904-8222-1c229aa0008d
ms.date: 07/18/2017
ms.localizationpriority: medium
ms.openlocfilehash: c6492e11c2ff39ddff77994a9321ae879c81c1d3
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/31/2019
ms.locfileid: "56578329"
---
# <a name="ndis-general-status-indications"></a>NDIS 一般状態表示





このセクションの内容:

[**NDIS\_状態\_メディア\_接続**](ndis-status-media-connect.md)

[**NDIS\_状態\_メディア\_切断**](ndis-status-media-disconnect.md)

[**NDIS\_状態\_リセット\_開始**](ndis-status-reset-start.md)

[**NDIS\_状態\_リセット\_終了**](ndis-status-reset-end.md)

[**NDIS\_状態\_メディア\_ビジー**](ndis-status-media-busy.md)

[**NDIS\_STATUS\_MEDIA\_SPECIFIC\_INDICATION**](ndis-status-media-specific-indication.md)

[**NDIS\_状態\_リンク\_速度\_変更**](ndis-status-link-speed-change.md)

[**NDIS\_状態\_リンク\_状態**](ndis-status-link-state.md)

[**NDIS\_状態\_ポート\_状態**](ndis-status-port-state.md)

[**NDIS\_状態\_工程\_状態**](ndis-status-oper-status.md)

[**NDIS\_状態\_ネットワーク\_変更**](ndis-status-network-change.md)

[**NDIS\_状態\_パケット\_フィルター**](ndis-status-packet-filter.md)

 

 




