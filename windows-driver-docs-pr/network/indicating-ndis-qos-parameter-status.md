---
title: NDIS QoS パラメーターの状態の表示
description: NDIS QoS パラメーターの状態の表示
...
ms.locfileid: "56581933"
---
# <a name="indicating-ndis-qos-parameter-status"></a>NDIS QoS パラメーターの状態の表示


IEEE 802.1 データ センター ブリッジング (DCB) インターフェイスの NDIS サービスの品質 (QoS) をサポートしているミニポート ドライバーは、次のイベントのいずれかが発生するたびに、NDIS 状態インジケーターを発行する必要があります。
