---
title: 仮想ポートの受信フィルターの管理
description: 仮想ポートの受信フィルターの管理
ms.assetid: 6FDBB159-3094-42F7-95EB-695B64BD4342
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 400bb72829a7cf101303db1ee94c9b54ad8ef06e
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/31/2019
ms.locfileid: "56572619"
---
# <a name="managing-the-receive-filters-for-a-virtual-port"></a>仮想ポートの受信フィルターの管理


受信フィルターをオフにしたセットを移動でき、VPort が作成された後、NIC スイッチの仮想ポート (VPort) で列挙されます。

ここでは、次のトピックについて説明します。

[仮想ポートで受信フィルターを設定](setting-a-receive-filter-on-a-virtual-port.md)

[仮想ポートで受信フィルターをクリア](clearing-a-receive-filter-on-a-virtual-port.md)

[仮想ポートに受信フィルターを移動](moving-a-receive-filter-to-a-virtual-port.md)

[受信仮想ポートのフィルターの列挙](enumerating-receive-filters-on-a-virtual-port.md)

拡張を作成する方法については、[仮想ポートを作成する](creating-a-virtual-port.md)を参照してください。

 

 





