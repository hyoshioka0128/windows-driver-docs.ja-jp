...
制御 (MAC) ヘッダーをデバイスが有効になっているまたはメディアで、VLAN 識別子を使用して、ネットワーク パケットをフィルター処理する機能を無効になっているかどうかを示す値にアクセスします。 ミニポート ドライバー、NDIS を使用してこの機能の報告\_受信\_フィルター\_MAC\_ヘッダー\_VLAN\_ID\_でサポートされているフラグ**SupportedMacHeaderFields**のメンバー、 [ **NDIS\_受信\_フィルター\_機能**](https://msdn.microsoft.com/library/windows/hardware/ff566864)構造体。
...
NDIS を呼び出すと、 [ *MiniportInitializeEx* ](https://msdn.microsoft.com/library/windows/hardware/ff559389)関数、ミニポート ドライバーこれらの手順に従う NDIS に現在有効になっている VMQ または RSS 機能を報告する前に。
