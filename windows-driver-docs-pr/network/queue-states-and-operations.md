...
上位のドライバーには、キューの割り当てが完了しました。 実行では、状態し、受信がキューに設定されているフィルターがある場合は、問題を開始できます。 キューの割り当てを完了する詳細については、次を参照してください。[割り当てと解放 VM キュー](allocating-and-freeing-vm-queues.md)します。
