...
-   基になるドライバーから OID 要求の応答をフィルター処理します。 OID 要求の詳細については、[フィルター モジュールの OID 要求](filter-module-oid-requests.md)を参照してください。

-   上にあるドライバーに状態インジケーターを生成します。

-   基になるドライバーからの状態インジケーターのフィルターを適用します。 詳細については、[フィルター モジュールの状態インジケーター](filter-module-status-indications.md)を参照してください。

-   そのインターフェイスを各ミニポート アダプター用にレジストリの構成パラメーターを管理します。 詳細については、[フィルター ドライバーの構成の情報にアクセスする](accessing-configuration-information-for-a-filter-driver.md)を参照してください。
