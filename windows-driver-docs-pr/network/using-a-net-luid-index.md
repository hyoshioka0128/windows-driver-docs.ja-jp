...
ネットワーク インターフェイスに登録するインターフェイスをプロバイダーが有効な NET を渡す必要があります\_LUID の値を[ **NdisIfRegisterInterface** ](https://msdn.microsoft.com/library/windows/hardware/ff562715)関数。 ネットワーク インターフェイスの登録の詳細については、[ネットワーク インターフェイスを登録する](registering-a-network-interface.md)を参照してください。
