...
システムと NIC の両方で wake on LAN がサポートされているかどうかを判断するために、NDIS は**systemwake**と**devicewake**の両方のメンバーを調べます。 **Systemwake**と**devicewake**の両方が**powersystemunspecified**に設定されている場合、NDIS は NIC を電源管理に対応するものとして扱います。 この場合、または、ミニポートドライバーによって NDIS\_属性が設定されていても、初期化中に\_SUSPEND フラグの\_\_停止\_が発生した場合、NDIS はその後ミニポートドライバーを発行します。これに[より、NIC](https://docs.microsoft.com/windows-hardware/drivers/network/oid-pnp-capabilities)のウェイクアップ機能に関する詳細情報を取得するように要求されます。\_\_
...
NDIS は、この情報を取得するとすぐに、ユーザーが UI で wake on LAN を有効にした場合に NIC を設定できるデバイスの電源状態を、システムの電源状態ごとに決定します。 許可されている低電力デバイスの状態によって、NIC がウェイクアップシグナルを生成できる場合 (つまり、デバイス\_機能の構造の**Devicestate**配列に指定されているすべての低電力デバイスの電源状態が、nic がシステムをスリープ解除できるデバイスの電源状態よりも低い場合)、NDIS によって、nic の **[電源管理]** タブの **[デバイスがスタンバイ]** 状態にならないようにする オプションが使用されます。 その後、ユーザーは wake on LAN を有効にできません。
...
2番目のオプションは、既定ではオフになっています。 NDIS によって、NIC がウェイクアップシグナルを生成することが許可されている低電力状態がないと判断した場合、NDIS は2番目のオプションを使用できません。 たとえば、デバイス\_機能構造の**Devicestate**配列メンバーが、すべての低電力システム状態に対して Nic が D3 である必要があることを示し、 **devicestate**によって nic がシステムのスリープ解除に使用できるデバイスの最低状態が D2 であることが示された場合、2番目のチェックボックスは使用できなくなります。
