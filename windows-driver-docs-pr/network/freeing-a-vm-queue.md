...
ミニポートドライバーが\_フィルターを使用して、\_QUEUE set 要求を[\_に受信する OID\_](https://docs.microsoft.com/windows-hardware/drivers/network/oid-receive-filter-free-queue)受信する場合、キューは、停止 dma 状態に入る必要があります。また、キューの dma を停止します。また、ミニポートドライバーは、 [**NDIS\_status\_RECEIVE\_queue\_state**](https://docs.microsoft.com/windows-hardware/drivers/network/ndis-status-receive-queue-state) status によって状態の変化を示す必要があります。 キューの状態の詳細については、「[キューの状態と操作](queue-states-and-operations.md)」を参照してください。
