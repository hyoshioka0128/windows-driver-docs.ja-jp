...
1.  ミニポートドライバーは、**ヘッダー**メンバーを初期化します。 ドライバーは、**ヘッダー**の**TYPE**メンバーを NDIS\_OBJECT\_TYPE\_QOS\_機能に設定します。
    ...
    NDIS 6.30 以降では、ミニポートドライバーは、**ヘッダー**の**リビジョン**メンバーを NDIS\_QOS\_機能\_リビジョン\_1、**サイズ**メンバーを ndis\_SIZEOF\_qos に設定し\_機能\_REVISION\_1 です。
...
3.  ネットワークアダプターが media access control security (MACsec) の処理をバイパスする機能をサポートしている場合、ミニポートドライバーは、**フラグ**でサポートされているフラグを\_バイパス\_\_MACSEC に NDIS\_QOS\_機能を設定します。レプリカ. MACsec の詳細については、「IEEE 802.1 AE-2006 標準」を参照してください。
