---
title: WDI ミニポート ドライバーの状態インジケーター
description: このセクションには、WDI 状態通知が含まれています。
ms.assetid: CD873C33-72ED-4F99-A584-5C307F4FB65E
ms.date: 07/18/2017
ms.localizationpriority: medium
ms.custom: 19H1
ms.openlocfilehash: 98b98138951000335e1447f230688a4a772ab531
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 04/23/2019
ms.locfileid: "63385367"
---
# <a name="wdi-miniport-driver-status-indications"></a>WDI ミニポート ドライバーの状態インジケーター


このセクションには、WDI 状態通知が含まれています。

## <a name="in-this-section"></a>このセクションの内容


-   [NDIS\_状態\_WDI\_INDICATION\_アクション\_フレーム\_受信日時](ndis-status-wdi-indication-action-frame-received.md)
-   [NDIS\_状態\_WDI\_INDICATION\_AP\_アソシエーション\_要求\_受信日時](ndis-status-wdi-indication-ap-association-request-received.md)
-   [NDIS\_状態\_WDI\_INDICATION\_アソシエーション\_パラメーター\_要求](ndis-status-wdi-indication-association-parameters-request.md)
-   [NDIS\_状態\_WDI\_INDICATION\_アソシエーション\_結果](ndis-status-wdi-indication-association-result.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_BSS\_ENTRY\_LIST](ndis-status-wdi-indication-bss-entry-list.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_CAN\_SUSTAIN\_AP](ndis-status-wdi-indication-can-sustain-ap.md)
-   [NDIS\_状態\_WDI\_INDICATION\_変更\_操作\_モード\_完了](ndis-status-wdi-indication-change-operation-mode-complete.md)
-   [NDIS_STATUS_WDI_INDICATION_CIPHER_KEY_UPDATED](ndis-status-wdi-indication-cipher-key-updated.md)
-   [NDIS\_状態\_WDI\_INDICATION\_閉じる\_完了](ndis-status-wdi-indication-close-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_CONNECT\_完了](ndis-status-wdi-indication-connect-complete.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_CREATE\_PORT\_COMPLETE](ndis-status-wdi-indication-create-port-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_削除\_ポート\_完了](ndis-status-wdi-indication-delete-port-complete.md)
-   [NDIS_STATUS_WDI_INDICATION_DEVICE_SERVICE_EVENT](ndis-status-wdi-indication-device-service-event.md)
-   [NDIS\_状態\_WDI\_INDICATION\_関連付けの解除](ndis-status-wdi-indication-disassociation.md)
-   [NDIS\_状態\_WDI\_INDICATION\_切断\_完了](ndis-status-wdi-indication-disconnect-complete.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_DOT11\_RESET\_COMPLETE](ndis-status-wdi-indication-dot11-reset-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_ファームウェア\_STALLED](ndis-status-wdi-indication-firmware-stalled.md)
-   [NDIS\_状態\_WDI\_INDICATION\_FT\_ASSOC\_PARAMS\_が必要](ndis-status-wdi-indication-ft-assoc-params-needed.md)
-   [NDIS\_状態\_WDI\_INDICATION\_IHV\_イベント](ndis-status-wdi-indication-ihv-event.md)
-   [NDIS\_状態\_WDI\_INDICATION\_IHV\_タスク\_完了](ndis-status-wdi-indication-ihv-task-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_IHV\_タスク\_要求](ndis-status-wdi-indication-ihv-task-request.md)
-   [NDIS\_状態\_WDI\_INDICATION\_リンク\_状態\_変更](ndis-status-wdi-indication-link-state-change.md)
-   [NDIS\_状態\_WDI\_INDICATION\_NLO\_検出](ndis-status-wdi-indication-nlo-discovery.md)
-   [NDIS\_状態\_WDI\_INDICATION\_オープン\_完了](ndis-status-wdi-indication-open-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_P2P\_アクション\_フレーム\_受信日時](ndis-status-wdi-indication-p2p-action-frame-received.md)
-   [NDIS\_状態\_WDI\_INDICATION\_P2P\_検出\_完了](ndis-status-wdi-indication-p2p-discovery-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_P2P\_グループ\_オペレーティング\_チャネル](ndis-status-wdi-indication-p2p-group-operating-channel.md)
-   [NDIS\_状態\_WDI\_INDICATION\_P2P\_オペレーティング\_チャネル\_属性](ndis-status-wdi-indication-p2p-operating-channel-attributes.md)
-   [NDIS\_状態\_WDI\_INDICATION\_P2P\_送信\_要求\_アクション\_フレーム\_完了](ndis-status-wdi-indication-p2p-send-request-action-frame-complete.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_P2P\_SEND\_RESPONSE\_ACTION\_FRAME\_COMPLETE](ndis-status-wdi-indication-p2p-send-response-action-frame-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_ラジオ\_状態](ndis-status-wdi-indication-radio-status.md)
-   [NDIS_STATUS_WDI_INDICATION_REQUEST_FTM_COMPLETE](ndis-status-wdi-indication-request-ftm-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_ローミング\_完了](ndis-status-wdi-indication-roam-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_ローミング\_が必要](ndis-status-wdi-indication-roaming-needed.md)
-   [NDIS_STATUS_WDI_INDICATION_SAE_AUTH_PARAMS_NEEDED](ndis-status-wdi-indication-sae-auth-params-needed.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_SCAN\_COMPLETE](ndis-status-wdi-indication-scan-complete.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_SEND\_AP\_ASSOCIATION\_RESPONSE\_COMPLETE](ndis-status-wdi-indication-send-ap-association-response-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_送信\_要求\_アクション\_フレーム\_完了](ndis-status-wdi-indication-send-request-action-frame-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_送信\_応答\_アクション\_フレーム\_完了](ndis-status-wdi-indication-send-response-action-frame-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_設定\_ラジオ\_状態\_完了](ndis-status-wdi-indication-set-radio-state-complete.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_START\_AP\_COMPLETE](ndis-status-wdi-indication-start-ap-complete.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_STOP\_AP](ndis-status-wdi-indication-stop-ap.md)
-   [NDIS\_STATUS\_WDI\_INDICATION\_STOP\_AP\_COMPLETE](ndis-status-wdi-indication-stop-ap-complete.md)
-   [NDIS\_状態\_WDI\_INDICATION\_タスク\_オフロード\_現在\_構成](ndis-status-wdi-indication-task-offload-current-config.md)
-   [NDIS\_状態\_WDI\_INDICATION\_TKIP\_MIC\_エラー](ndis-status-wdi-indication-tkip-mic-failure.md)
-   [NDIS\_状態\_WDI\_INDICATION\_WAKE\_理由](ndis-status-wdi-indication-wake-reason.md)

 

 




