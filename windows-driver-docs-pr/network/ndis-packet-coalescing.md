---
title: NDIS パケット結合
description: NDIS パケット結合
ms.assetid: 500FBF0F-54D9-4675-8E2D-447387DA8798
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 849c6d8e911be28efbbe8f65272e23df5586132e
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 04/23/2019
ms.locfileid: "63378290"
---
# <a name="ndis-packet-coalescing"></a>NDIS パケット結合


NDIS パケット結合 NDIS 6.30 以降では、ネットワーク アダプターをサポートできます。 この機能は、処理を軽減ランダム ブロードキャストまたはマルチキャスト パケットの受信のためのホスト システム上のオーバーヘッドと電力の消費量。

ここでは、次のトピックについて説明します。

[パケットの結合の概要](overview-of-packet-coalescing.md)

[レポートのパケットの結合機能](reporting-packet-coalescing-capabilities.md)

[クエリ パケット結合機能](querying-packet-coalescing-capabilities.md)

[フィルターは受信パケットの結合を管理します。](managing-packet-coalescing-receive-filters.md)

[パケットの結合の標準化された INF キーワード](standardized-inf-keywords-for-packet-coalescing.md)

 

 





