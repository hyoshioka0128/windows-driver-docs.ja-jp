---
title: MB NDIS 状態通知
description: MB NDIS 状態通知
ms.assetid: 97538f74-f602-4d38-a5a8-e52eb3194fb9
ms.date: 07/18/2017
ms.localizationpriority: medium
ms.custom: 19H1
ms.openlocfilehash: a929922a6a67b2e9d618308e187514af2c1254bb
ms.sourcegitcommit: 0cc5051945559a242d941a6f2799d161d8eba2a7
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 04/23/2019
ms.locfileid: "63343316"
---
# <a name="mb-ndis-status-notifications"></a>MB NDIS 状態通知


このセクションには、NDIS ミニポート ドライバーのモバイル ブロード バンド (MB) の状態の通知が含まれています。

[**NDIS\_STATUS\_WWAN\_AUTH\_RESPONSE**](ndis-status-wwan-auth-response.md)

[**NDIS_STATUS_WWAN_BASE_STATIONS_INFO**](ndis-status-wwan-base-stations-info.md)

[**NDIS\_状態\_WWAN\_コンテキスト\_状態**](ndis-status-wwan-context-state.md)

[**NDIS\_状態\_WWAN\_デバイス\_キャップ**](ndis-status-wwan-device-caps.md)

[**NDIS\_状態\_WWAN\_デバイス\_CAP\_例**](ndis-status-wwan-device-caps-ex.md)

[**NDIS_STATUS_WWAN_DEVICE_RESET_STATUS](ndis-status-wwan-device-reset-status.md)

[**NDIS\_状態\_WWAN\_デバイス\_サービス\_イベント**](ndis-status-wwan-device-service-event.md)

[**NDIS\_状態\_WWAN\_デバイス\_サービス\_応答**](ndis-status-wwan-device-service-response.md)

[**NDIS\_状態\_WWAN\_デバイス\_サービス\_セッション**](ndis-status-wwan-device-service-session.md)

[**NDIS\_状態\_WWAN\_デバイス\_サービス\_セッション\_読み取り**](ndis-status-wwan-device-service-session-read.md)

[**NDIS\_状態\_WWAN\_デバイス\_サービス\_セッション\_書き込み\_完了**](ndis-status-wwan-device-service-session-write-complete.md)

[**NDIS\_状態\_WWAN\_デバイス\_サービス\_サブスクリプション**](ndis-status-wwan-device-service-subscription.md)

[**NDIS\_状態\_WWAN\_デバイス\_サービス\_サポートされている\_コマンド**](ndis-status-wwan-device-service-supported-commands.md)

[**NDIS\_状態\_WWAN\_デバイス\_スロット\_マッピング**](ndis-status-wwan-device-slot-mappings.md)

[**NDIS\_状態\_WWAN\_ホーム\_プロバイダー**](ndis-status-wwan-home-provider.md)

[**NDIS_STATUS_WWAN_IP_ADDRESS_STATE**](ndis-status-wwan-ip-address-state.md)

[**NDIS_STATUS_WWAN_LTE_ATTACH_CONFIG**](ndis-status-wwan-lte-attach-config.md)

[**NDIS_STATUS_WWAN_LTE_ATTACH_STATUS**](ndis-status-wwan-lte-attach-status.md)

[**NDIS_STATUS_WWAN_MODEM_CONFIG_INFO**](ndis-status-wwan-modem-config-info.md)

[**NDIS_STATUS_WWAN_MODEM_LOGGING_CONFIG**](ndis-status-wwan-modem-logging-config.md)

[**NDIS_STATUS_WWAN_MPDP_LIST**](ndis-status-wwan-mpdp-list.md)

[**NDIS_STATUS_WWAN_MPDP_STATE**](ndis-status-wwan-mpdp-state.md)

[**NDIS_STATUS_WWAN_NETWORK_BLACKLIST**](ndis-status-wwan-network-blacklist.md)

[**NDIS_STATUS_WWAN_NITZ_INFO**](ndis-status-wwan-nitz-info.md)

[**NDIS\_状態\_WWAN\_パケット\_サービス**](ndis-status-wwan-packet-service.md)

[**NDIS_STATUS_WWAN_PCO_STATUS**](ndis-status-wwan-pco-status.md)

[**NDIS\_状態\_WWAN\_PIN\_情報**](ndis-status-wwan-pin-info.md)

[**NDIS\_状態\_WWAN\_PIN\_一覧**](ndis-status-wwan-pin-list.md)

[**NDIS\_状態\_WWAN\_優先\_マルチ\_プロバイダー**](ndis-status-wwan-preferred-multicarrier-providers.md)

[**NDIS\_状態\_WWAN\_優先\_プロバイダー**](ndis-status-wwan-preferred-providers.md)

[**NDIS\_状態\_WWAN\_PRESHUTDOWN\_状態**](ndis-status-wwan-preshutdown-state.md)

[**NDIS\_状態\_WWAN\_プロビジョニング済み\_コンテキスト**](ndis-status-wwan-provisioned-contexts.md)

[**NDIS\_状態\_WWAN\_ラジオ\_状態**](ndis-status-wwan-radio-state.md)

[**NDIS\_状態\_WWAN\_準備\_情報**](ndis-status-wwan-ready-info.md)

[**NDIS\_状態\_WWAN\_登録\_状態**](ndis-status-wwan-register-state.md)

[**NDIS_STATUS_WWAN_SAR_CONFIG**](ndis-status-wwan-sar-config.md)

[**NDIS_STATUS_WWAN_SAR_TRANSMISSION_STATUS**](ndis-status-wwan-sar-transmission-status.md)

[**NDIS\_状態\_WWAN\_サービス\_アクティブ化**](ndis-status-wwan-service-activation.md)

[**NDIS\_状態\_WWAN\_設定\_ホーム\_プロバイダー\_完了**](ndis-status-wwan-set-home-provider-complete.md)

[**NDIS\_状態\_WWAN\_信号\_状態**](ndis-status-wwan-signal-state.md)

[**NDIS\_STATUS\_WWAN\_SLOT\_INFO\_STATUS**](ndis-status-wwan-slot-info-status.md)

[**NDIS\_状態\_WWAN\_SMS\_構成**](ndis-status-wwan-sms-configuration.md)

[**NDIS\_状態\_WWAN\_SMS\_削除**](ndis-status-wwan-sms-delete.md)

[**NDIS\_状態\_WWAN\_SMS\_受信**](ndis-status-wwan-sms-receive.md)

[**NDIS\_状態\_WWAN\_SMS\_送信**](ndis-status-wwan-sms-send.md)

[**NDIS\_状態\_WWAN\_SMS\_状態**](ndis-status-wwan-sms-status.md)

[**NDIS\_状態\_WWAN\_サポートされている\_デバイス\_サービス**](ndis-status-wwan-supported-device-services.md)

[**NDIS\_状態\_WWAN\_SYS\_キャップ**](ndis-status-wwan-sys-caps.md)

[**NDIS_STATUS_WWAN_UICC_APP_LIST**](ndis-status-wwan-uicc-app-list.md)

[**NDIS_STATUS_WWAN_UICC_BINARY_RESPONSE**](ndis-status-wwan-uicc-binary-response.md)

[**NDIS_STATUS_WWAN_UICC_FILE_STATUS**](ndis-status-wwan-uicc-file-status.md)

[**NDIS_STATUS_WWAN_UICC_RECORD_RESPONSE**](ndis-status-wwan-uicc-record-response.md)

[**NDIS_STATUS_WWAN_UICC_RESET_INFO**](ndis-status-wwan-uicc-reset-info.md)

[**NDIS\_状態\_WWAN\_USSD**](ndis-status-wwan-ussd.md)

[**NDIS\_状態\_WWAN\_ベンダー\_特定**](ndis-status-wwan-vendor-specific.md)

[**NDIS\_状態\_WWAN\_VISIBLE\_プロバイダー**](ndis-status-wwan-visible-providers.md)
