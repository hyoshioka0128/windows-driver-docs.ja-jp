...
KSNODETYPE\_MUX ノードは、マルチプレクサー (マルチプレクサー) を表します。 MUX は、複数の入力ストリームと、同じデータ形式をすべて 1 つの出力ストリームがあります。 一度に 1 つだけの入力ストリームは、出力ストリームにルーティングされます。

KSNODETYPE\_MUX ノードは、次の必要なプロパティをサポートする必要があります。
