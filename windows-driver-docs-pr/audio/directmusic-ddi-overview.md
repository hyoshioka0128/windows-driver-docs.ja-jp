...
最初に、DirectMusic のソフトウェアの実装を記述する最適な設計戦略では通常、*デバイス ドライバー インターフェイス (DDI)* ユーザー モードで実行します。 最終的な製品がカーネル モードの実装のハードウェア コンポーネントを使用する場合でも、このアプローチの使用をお勧めします。 ユーザー モードのバージョンが完了すると、ソフトウェアは、カーネル モード ドライバーとハードウェア、一度に 1 つの機能によって確立された接続に変換できます。 詳細については、[ユーザー モードとカーネル モード](user-mode-versus-kernel-mode.md)を参照してください。
