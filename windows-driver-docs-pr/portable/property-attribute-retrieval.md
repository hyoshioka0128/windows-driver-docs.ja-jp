...
ms.locfileid: "56572564"
---
# <a name="property-attribute-retrieval"></a>プロパティ属性の取得


WPD アプリケーションを呼び出すと、 **IPortableDeviceProperties::GetPropertyAttributes**メソッドでは、このメソッドは、さらへの呼び出しをトリガー、 **WpdObjectProperties::OnGetPropertyAttributes**メソッドサンプル ドライバー。 後者のメソッドを作成、 **IPortableDeviceValues**にドライバーが指定されたプロパティの属性を格納するオブジェクト。
