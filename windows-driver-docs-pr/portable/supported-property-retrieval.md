---
Description: サポートされているプロパティの取得
title: サポートされているプロパティの取得
ms.date: 04/20/2017
...
ms.locfileid: "63362456"
---
# <a name="supported-property-retrieval"></a>サポートされているプロパティの取得


WPD アプリケーションを呼び出すと、 **IPortableDeviceProperties::GetSupportedProperties**メソッドでは、このメソッドは、さらへの呼び出しをトリガー、 **WpdObjectProperties::OnGetSupportedProperties**ドライバーのサンプルのメソッドです。 後者のメソッドを作成、 **IPortableDeviceKeyCollection**ドライバーのプロパティの格納先のオブジェクトは、特定のオブジェクトでサポートされるプロパティのキーします。
