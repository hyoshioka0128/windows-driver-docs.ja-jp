---
title: KSK プロパティ\_CAMERACONTROL\_ROLL\_相対
description: KSK プロパティ\_CAMERACONTROL\_ROLL\_相対プロパティは、画像表示軸を中心としたカメラの回転を指定します。
ms.assetid: cd16369b-558c-48b6-9a0a-ffd4e4561a30
keywords:
- KSPROPERTY_CAMERACONTROL_ROLL_RELATIVE ストリーミングメディアデバイス
topic_type:
- apiref
api_name:
...
ms.locfileid: "72842484"
---
# <a name="ksproperty_cameracontrol_roll_relative"></a>KSK プロパティ\_CAMERACONTROL\_ROLL\_相対


KSK プロパティ\_CAMERACONTROL\_ROLL\_相対プロパティは、画像表示軸を中心としたカメラの回転を指定します。
...
プロパティ値 (操作データ) は、カメラの相対ロール設定を指定する LONG です。 値のサイズは、必要な回転速度を表します。値が大きいほど、より高速になります。
...
<tr class="even">
<td><p>正の値</p></td>
<td><p>表示軸の周りを時計回りに回転します。</p></td>
</tr>
<tr class="odd">
<td><p>負の値</p></td>
<td><p>表示軸の周りを反時計回りに回転させます。</p></td>
</tr>
</tbody>
</table>
...
[**Ksk プロパティ\_CAMERACONTROL\_NODE\_S**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ksmedia/ns-ksmedia-ksproperty_cameracontrol_node_s)構造体の**値**メンバーは、相対ロールを指定します。
...
一部のデバイスでは、1つの回転速度しかサポートしていません。 この場合、**値**メンバーの符号は、回転の方向を示します。

クライアントは、set 要求を行うときに、前の表の値のいずれかを\_CAMERACONTROL\_NODE\_S 構造体の KSK プロパティの**値**のメンバーに指定する必要があります。

Get 要求を行うと、クライアントは、前の表の値のいずれかを、\_CAMERACONTROL\_NODE\_S 構造体の KSK プロパティの**値**メンバーで受け取ります。 値は、カメラの現在の回転状態を示します。
