---
title: デバイスに固有のコマンド コード
description: デバイスに固有のコマンド コード
ms.assetid: 5dae7269-7a5d-4e34-b164-cfb02f17ee2c
ms.date: 11/28/2017
ms.localizationpriority: medium
ms.openlocfilehash: b01c7e14be699383ff624617d6200cd0ede3f6d9
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/31/2019
ms.locfileid: "56558667"
---
# <a name="device-specific-command-codes"></a>デバイスに固有のコマンド コード


## <span id="ddk_device_specific_command_codes_ks"></span><span id="DDK_DEVICE_SPECIFIC_COMMAND_CODES_KS"></span>


次の SRB コマンドでは、特定のデバイスがあります。

[**SRB\_変更\_POWER\_状態**](srb-change-power-state.md)

[**SRB\_閉じる\_デバイス\_インスタンス**](srb-close-device-instance.md)

[**SRB\_閉じる\_ストリーム**](srb-close-stream.md)

[**SRB\_取得\_データ\_積集合**](srb-get-data-intersection.md)

[**SRB\_取得\_デバイス\_プロパティ**](srb-get-device-property.md)

[**SRB\_GET\_STREAM\_INFO**](srb-get-stream-info.md)

[**SRB\_初期化\_完了**](srb-initialization-complete.md)

[**SRB\_初期化\_デバイス**](srb-initialize-device.md)

[**SRB\_通知\_IDLE\_状態**](srb-notify-idle-state.md)

[**SRB\_オープン\_デバイス\_インスタンス**](srb-open-device-instance.md)

[**SRB\_OPEN\_STREAM**](srb-open-stream.md)

[**SRB\_ページング\_アウト\_ドライバー**](srb-paging-out-driver.md)

[**SRB\_設定\_デバイス\_プロパティ**](srb-set-device-property.md)

[SRB\_突然\_削除](srb-surprise-removal.md)

[**SRB\_非\_デバイス**](srb-uninitialize-device.md)

[SRB\_不明な\_デバイス\_コマンド](srb-unknown-device-command.md)

 

 





