...
縦のプライマリデバイスにマウントする場合、カメラドライバーは、実際のカメラの出力バッファーの向きに関係なく、ほとんどのアプリケーションがデバイスを横向きカメラの出力バッファーの出力として扱うことに注意する必要があります。 このため、縦向きのプライマリデバイスでは、カメラのドライバーが、横方向の縦向きから90度の向きオフセットを持つカメラバッファーを出力することをお勧めします。 これにより、縦のデバイスでこの追加の回転を実行しているアプリケーションが、予想される向きまで回転を修正できるようになります。 これは、[カメラアプリケーションとローテーションサンプル](https://github.com/microsoft/Windows-universal-samples/tree/master/Samples/CameraStarterKit/cs)を使用して確認できます。
...
ユーザーモードで修正を実行し、パイプラインとユーザーモードプラグイン間の API コントラクトを従う必要があることを強くお勧めします。 具体的には、DMFT または MFT0 のいずれかを使用しているときに、IMFDeviceTransform::P Roて Message または IMFTransform::P Roて message が MFT\_メッセージ\_\_設定されている場合、ユーザーモードプラグインは次のガイドラインに従う必要があります。\_
