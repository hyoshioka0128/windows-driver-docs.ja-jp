---
title: コーデックのメリットの検証
description: コーデックのメリットの検証
ms.assetid: 595a21ea-bae2-410e-874a-7f3429ac02a1
keywords:
- ハードウェア コーデック WDK AVStream、メリットを検証しています
- WDK AVStream のコーデック
- コーデック WDK AVStream、価値の検証
- WDK AVStream、ハードウェアのコーデック
- コーデック WDK AVStream、ハードウェアを検証するだけの価値します。
ms.date: 04/20/2017
...
ms.locfileid: "63384156"
---
# <a name="codec-merit-validation"></a>コーデックのメリットの検証


KSPROPSETID をサポートするために\_OPMVideoOutput、次のメソッドを実装します。

-   KSMETHOD\_OPMVIDEOOUTPUT\_STARTINITIALIZATION

    [IOPMVideoOutput::StartInitialization](https://go.microsoft.com/fwlink/p/?linkid=155056) KSMETHOD によってドライバーへのプロキシ\_OPMVIDEOOUTPUT\_STARTINITIALIZATION します。

-   KSMETHOD\_OPMVIDEOOUTPUT\_ENDINITIALIZATION

    [IOPMVideoOutput::FinishInitialization](https://go.microsoft.com/fwlink/p/?linkid=155053) KSMETHOD によってドライバーへのプロキシ\_OPMVIDEOOUTPUT\_ENDINITIALIZATION します。

-   KSMETHOD\_OPMVIDEOOUTPUT\_のために GETINFORMATION

    [IOPMVideoOutput::GetInformation](https://go.microsoft.com/fwlink/p/?linkid=155054) KSMETHOD によってドライバーへのプロキシ\_OPMVIDEOOUTPUT\_のために GETINFORMATION します。

このトピックの残りの部分には、これら 3 つのメソッドの実装例が含まれています。

このコードを使用するには、次のヘッダー ファイルを含める必要があります<em>:</em>
...
*Testcert.h*の内容は、このトピックの下部には、サンプル テスト証明書は、します。 *Testcert.h*は、ドライバーが運用環境の独自の証明書を取得する必要があります。 テストのみを目的と、リリース ビルドでは機能しません。

追加する必要がありますも*Ksecdd.lib*の TARGETLIBS セクションに、*ソース*ファイル。
