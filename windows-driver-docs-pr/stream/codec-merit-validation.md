---
title: コーデックのメリットの検証
description: コーデックの価値検証
ms.assetid: 595a21ea-bae2-410e-874a-7f3429ac02a1
keywords:
- ハードウェアコーデック WDK AVStream、評価価値
- コーデック WDK AVStream
- コーデック WDK AVStream、価値検証
- コーデック WDK AVStream、ハードウェア
- コーデック WDK AVStream、ハードウェア、価値検証
ms.date: 06/16/2020
...
ms.locfileid: "84879038"
---
# <a name="codec-merit-validation"></a>コーデックの価値検証

KSPROPSETID OPMVideoOutput をサポートするには \_ 、次のメソッドを実装します。

- KSMETHOD \_ OPMVIDEOOUTPUT \_ STARTINITIALIZATION

    IOPMVideoOutput メソッド OPMVIDEOOUTPUT startinitialization を使用した、ドライバーへの[IOPMVideoOutput:: startinitialization](https://docs.microsoft.com/windows/win32/api/opmapi/nf-opmapi-iopmvideooutput-startinitialization)プロキシ \_ \_ 。

- KSMETHOD \_ OPMVIDEOOUTPUT \_ ENDINITIALIZATION

    [IOPMVideoOutput::](https://docs.microsoft.com/windows/win32/api/opmapi/nf-opmapi-iopmvideooutput-finishinitialization) ksk メソッド OPMVIDEOOUTPUT endinitialization を使用してドライバーへの初期化プロキシを終了 \_ \_ します。

- KSMETHOD \_ OPMVIDEOOUTPUT \_ GETINFORMATION

    [IOPMVIDEOOUTPUT](https://docs.microsoft.com/windows/win32/api/opmapi/nf-opmapi-iopmvideooutput-getinformation)メソッド OPMVIDEOOUTPUT GetInformation を使用して、ドライバーに対して IOPMVideoOutput プロキシを実行します \_ \_ 。

このトピックの残りの部分では、これら3つのメソッドの実装例を示します。

このコードを使用するには、次のヘッダーファイルをインクルードする必要があります。
...
*Testcert .h*は、このトピックの下部にあるサンプルテスト証明書です。 *Testcert .h*はテスト目的でのみ使用でき、リリースビルドでは機能しません。実稼働ドライバー用の独自の証明書を取得する必要があります。

また、次のように、*ソース*ファイルの*Targetlibs セクションに ksecdd .lib*を追加する必要があります。
