...
に対して次の制限、 [ **DXVA\_ConnectMode** ](https://msdn.microsoft.com/library/windows/hardware/ff563138)構造体に適用されるときに、 *bDXVA\_Func* で定義された変数**dwFunction**のメンバー、 [ **DXVA\_ConfigPictureDecode** ](https://msdn.microsoft.com/library/windows/hardware/ff563133)構造体が 1 の (画像をデコード) または 4 (画像再サンプリングします)。
...
次の制限、 [ **DXVA\_PicResample** ](https://msdn.microsoft.com/library/windows/hardware/ff564010)構造体は、ときに適用、 [bDXVA\_Func 変数](bdxva-func-variable.md)で定義されています。**dwFunction**のメンバー、 [ **DXVA\_ConfigPictureDecode** ](https://msdn.microsoft.com/library/windows/hardware/ff563133)構造が 4 に等しい。
...
**注**  が H.263 のサポートのみが必要です、 **bBidirectionalAveragingMode**のメンバー [ **DXVA\_PictureParameters** ](https://msdn.microsoft.com/library/windows/hardware/ff564012) 1 に等しい*MotionForward*は 1 と*MotionBackward*は 1、H263 です\_もにより、D の制限プロファイル**bBidirectionalAveragingMode。** を 0 にします。 これは、H263 を許可するものです\_D mpeg-4 ビデオと H.263 ビデオをサポートするためにプロファイルを制限する (mpeg-4 では、双方向の平均の MPEG 1/mpeg-2 スタイルを使用します)。
