...
H263\_B の制限付きプロファイルには、ITU-T のサポートに必要な一連の機能と、一連の拡張オプション機能が含まれています。 このプロファイルのサポートは現在推奨されていますが、必須ではありません。 この一連の機能は、制限されたプロファイル[\_H263](h263-a.md)に記載されている制限によって指定されます。ただし、次の制限があります。
...
[**DXVA\_Configpicture デコード**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dxva/ns-dxva-_dxva_configpicturedecode)の**dwfunction**メンバーで定義されている*bDXVA\_Func*変数が 1 (画像のデコード) の場合、 [**DXVA\_connectmode**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dxva/ns-dxva-_dxva_connectmode)構造に関する次の制限が適用されます。).
...
[**DXVA\_Configピクチャデコード**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dxva/ns-dxva-_dxva_configpicturedecode)の**dwfunction**メンバーで定義されている[bDXVA\_Func 変数](bdxva-func-variable.md)が1に等しい場合、 [**DXVA\_ピクチャパラメーター**](https://docs.microsoft.com/windows-hardware/drivers/ddi/dxva/ns-dxva-_dxva_pictureparameters)構造体に関する次の制限が適用されます。
...
<td align="left"><p>0または1に等しい</p></td>
</tr>
<tr class="even">
...
<td align="left"><p>値は 0 (ジグザグ)、値 1 (代替垂直)、または値 2 (水平方向) ( <strong>bConfigHostInverseScan</strong>が0の場合) です。</p></td>
</tr>
<tr class="even">
<td align="left"><p><strong>wMBtype</strong></p></td>
<td align="left"><p>この構造体のメンバーに含まれている<em>Motion4MV</em>フラグは、0または1に等しくなります。</p></td>
</tr>
</tbody>
</table>
...
すべてのビットストリームバッファーの内容には、ビデオ形式のデータを含めることもできます。これには、CPCF、CPFMT、および発表 Exe D、I、N (出力画像ごとの単一前方参照画像)、T があります。
