...
**PpThis**保護モードに設定されている COPP DirectX VA デバイス オブジェクトへのポインターへのポインターします。 **InputBuffer**メンバーへのポインターに設定されます、 [ **DXVA\_COPPSignature** ](https://msdn.microsoft.com/library/windows/hardware/ff563150)保護されたセッションを開始する構造体。 **Phr**メンバーから返される値に設定する必要があります、 [ *COPPSequenceStart* ](https://msdn.microsoft.com/library/windows/hardware/ff540421)関数。
