...
ms.locfileid: "63370158"
---
# <a name="providing-capabilities-for-video-processing"></a>ビデオ処理の機能の提供


ときにその[ **GetCaps** ](https://msdn.microsoft.com/library/windows/hardware/ff566762)関数が呼び出されると、ユーザー モードのディスプレイ ドライバーが、要求の種類に基づいて次のビデオの処理機能を提供します (で指定される、 **の種類**のメンバー、 [ **D3DDDIARG\_GETCAPS** ](https://msdn.microsoft.com/library/windows/hardware/ff543148)構造体、 *pData*パラメーターが指す)。
...
ユーザー モードのディスプレイ ドライバーは、数とビデオの処理をサポートしている次の Guid のリストを返します。 マイクロソフトの Direct3D ランタイムを指定します、 [ **DXVADDI\_VIDEODESC** ](https://msdn.microsoft.com/library/windows/hardware/ff562944)変数で処理する特定のビデオ ストリームの構造を**pInfo**メンバーの D3DDDIARG\_GETCAPS が指します。 ランタイムは、まずサポートされている Guid の後にサポートされている Guid の一覧については、要求の数を要求します。
...
各ビデオ プロセッサ モード、ユーザー モードのディスプレイ ドライバーをサポートするには、固有の機能を持つことができます。 ユーザー モードのディスプレイ ドライバーがこれらの機能を返すときに、D3DDDICAPS\_GETVIDEOPROCESSORCAPS 要求の種類が渡されます。 Direct3D のランタイムを指定します、 [ **DXVADDI\_VIDEOPROCESSORINPUT** ](https://msdn.microsoft.com/library/windows/hardware/ff562956)変数の機能を取得するビデオ処理モードの構造を**pInfo**のメンバー [ **D3DDDIARG\_GETCAPS** ](https://msdn.microsoft.com/library/windows/hardware/ff543148)を指します。 ユーザー モードのディスプレイ ドライバーがビデオ処理モードの機能を返します、 [ **DXVADDI\_VIDEOPROCESSORCAPS** ](https://msdn.microsoft.com/library/windows/hardware/ff562953)構造体、 **pData**メンバーの D3DDDIARG\_GETCAPS が指します。
...
ユーザー モードのディスプレイ ドライバーへのポインターを返します、 [ **DXVADDI\_VALUERANGE** ](https://msdn.microsoft.com/library/windows/hardware/ff562939)の範囲を含む構造体では、特定の ProcAmp の特定のコントロール プロパティの値が許可されています。ビデオ ストリーム。 Direct3D のランタイムを指定します、 [ **DXVADDI\_QUERYPROCAMPINPUT** ](https://msdn.microsoft.com/library/windows/hardware/ff562935) ProcAmp コントロール プロパティの変数に特定のビデオ ストリームの構造体を**pInfo** D3DDDIARG のメンバー\_GETCAPS を指します。
...
ユーザー モードでは、ドライバーの取得数と、一覧の表示、特定のビデオの処理モードをサポートしているターゲットの形式を表示します。 Direct3D のランタイムを指定します、 [ **DXVADDI\_VIDEOPROCESSORINPUT** ](https://msdn.microsoft.com/library/windows/hardware/ff562956)変数にビデオ プロセッサ モードの構造を**pInfo**のメンバーD3DDDIARG\_GETCAPS を指します。 ユーザー モードのディスプレイ ドライバー返しますレンダリング ターゲットの形式の配列でサポートされる[ **D3DDDIFORMAT**](https://msdn.microsoft.com/library/windows/hardware/ff544312)-型指定された値を**pData** D3DDDIARG のメンバー\_GETCAPS を指定します。
...
ユーザー モードは、数と、特定のビデオの処理モードをサポートしているサブ stream の形式の一覧に、ドライバーを返します。 を表示します。 Direct3D のランタイムを指定します、 [ **DXVADDI\_VIDEOPROCESSORINPUT** ](https://msdn.microsoft.com/library/windows/hardware/ff562956)変数にビデオ プロセッサ モードの構造を**pInfo**のメンバーD3DDDIARG\_GETCAPS を指します。 ユーザー モードのディスプレイ ドライバーの配列でサポートされているサブ stream の形式を返します[ **D3DDDIFORMAT**](https://msdn.microsoft.com/library/windows/hardware/ff544312)-型指定された値を**pData** D3DDDIARG のメンバー\_GETCAPS を指定します。
...
ユーザー モードのディスプレイ ドライバーへのポインターを返します、 [ **DXVADDI\_VALUERANGE** ](https://msdn.microsoft.com/library/windows/hardware/ff562939)の範囲を含む構造体では、特定のビデオ ストリームで特定のフィルター設定の値が許可されています。ときに、D3DDDICAPS\_FILTERPROPERTYRANGE 要求の種類が渡されます。 Direct3D のランタイムを指定します、 [ **DXVADDI\_QUERYFILTERPROPERTYRANGEINPUT** ](https://msdn.microsoft.com/library/windows/hardware/ff562930)変数内の特定のビデオ ストリームのフィルター設定の構造を**pInfo** D3DDDIARG のメンバー\_GETCAPS を指します。
