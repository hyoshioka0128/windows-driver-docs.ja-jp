...
内で**DrvCreateDeviceBitmap**、ドライバーは、GDI サービスを呼び出して[ **EngCreateDeviceBitmap** ](https://docs.microsoft.com/windows/desktop/api/winddi/nf-winddi-engcreatedevicebitmap) GDI のデバイスのビットマップのハンドルを作成します。
...
ドライバーは、ビットマップを作成する場合にできる必要がありますも実装することでそれらを削除する、 [ **DrvDeleteDeviceBitmap** ](https://docs.microsoft.com/windows/desktop/api/winddi/nf-winddi-drvdeletedevicebitmap)関数。
