...
このセクションでは、マイクロソフトの Direct3D バージョン 11.0 のランタイムに追加されるユーザー モード ディスプレイ ドライバー状態関数について説明します。 それ以外の場合、ユーザー モードの残りの部分では、ドライバーの状態の関数を表示を参照してください[Direct3D のバージョン 10 状態関数](#direct3d-version-10-state-functions)と[Direct3D バージョン 10.1 状態関数](#direct3d-version-10-1-state-functions)します。
