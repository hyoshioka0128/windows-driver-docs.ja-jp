...
次のコード例は、ドライバーの使用方法を示しています、**try/except**無効なメモリにアクセスするために、エラーが発生した場合に例外をスローするためのメカニズムです。
...
**注**  とは別にアクセスして、ローカル変数にユーザー モードの値をコピー、ドライバーを行わないで内で他の操作、  **\_\_try**ブロックします。 その他の操作には、独自の例外が発生する可能性があります。 これらの例外の処理は、オペレーティング システムによって異なります。
