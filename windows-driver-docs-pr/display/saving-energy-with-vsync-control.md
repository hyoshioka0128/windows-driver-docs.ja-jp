...
ドライバーはこの機能を活用するために、それらをサポートする必要があります、 **VSyncPowerSaveAware**内のメンバー、 [ **DXGK\_VIDSCHCAPS** ](https://msdn.microsoft.com/library/windows/hardware/ff562863)が構造Windows Vista SP1 で導入されました。 WDDM に従って既存のドライバーを再コンパイルする必要があります、 **VSyncPowerSaveAware**メンバーは、Windows Server 2008 またはそれ以降のバージョンの WDK を使用します。
...
Windows 8 以降は省略可能、[カーネル モード表示専用ドライバー (KMDOD)](https://msdn.microsoft.com/library/windows/hardware/jj673962)次のように、垂直同期機能をサポートするために。
...
両方に実装する必要があります、KMDOD 垂直同期の制御機能をサポートする場合[ *DxgkDdiControlInterrupt* ](https://msdn.microsoft.com/library/windows/hardware/ff559602)と[ *DxgkDdiGetScanLine* ](https://msdn.microsoft.com/library/windows/hardware/ff559668)関数し、でこれらの関数の両方に有効な関数ポインターを提供する必要があります、 [ **KMDDOD\_初期化\_データ**](https://msdn.microsoft.com/library/windows/hardware/hh451571)構造体。

この場合、KMDOD を実装する必要がありますも、 [ *DxgkDdiInterruptRoutine* ](https://msdn.microsoft.com/library/windows/hardware/ff559680)と[ *DxgkDdiDpcRoutine* ](https://msdn.microsoft.com/library/windows/hardware/ff559645)関数にはオペレーティング システムには、垂直同期の割り込みを報告します。

さらの値、 **PixelRate**、 **hSyncFreq**、および**vSyncFreq**のメンバー、 [ **DISPLAYCONFIG\_ビデオ\_信号\_情報**](https://msdn.microsoft.com/library/windows/hardware/ff554007)構造にすることはできません**D3DKMDT\_頻度\_NOTSPECIFIED**します。
...
場合は、KMDOD 垂直同期の制御機能をサポートしていない必要がありますを実装していないか[ *DxgkDdiControlInterrupt* ](https://msdn.microsoft.com/library/windows/hardware/ff559602)または[ *DxgkDdiGetScanLine* ](https://msdn.microsoft.com/library/windows/hardware/ff559668)関数し、でこれらの関数のいずれかに有効な関数ポインターを指定しないで、 [ **KMDDOD\_初期化\_データ**](https://msdn.microsoft.com/library/windows/hardware/hh451571)構造体。

ここで、Microsoft DirectX グラフィックスのカーネル サブシステムは、垂直同期の割り込みと現在のモードとシミュレートされた最後の垂直同期の時刻に基づくスキャン ラインの値をシミュレートします。

さらの値、 **PixelRate**、 **hSyncFreq**、および**vSyncFreq**のメンバー、 [ **DISPLAYCONFIG\_ビデオ\_信号\_情報**](https://msdn.microsoft.com/library/windows/hardware/ff554007)に構造体を設定する必要があります**D3DKMDT\_頻度\_NOTSPECIFIED**します。
