---
title: ユーザー モードのディスプレイ ドライバーによって呼び出された D3D ランタイム関数
description: このトピックでは、マイクロソフトの Direct3D ランタイムがユーザー モードのディスプレイ ドライバーを提供する関数を参照します。
...
ms.locfileid: "56539190"
---
# <a name="direct3d-runtime-functions-called-by-user-mode-display-drivers"></a>ユーザー モードのディスプレイ ドライバーによって呼び出された Direct3D ランタイム関数

このトピックでは、マイクロソフトの Direct3D ランタイムがユーザー モードのディスプレイ ドライバーを提供する関数を参照します。 Direct3D ランタイム カーネル サービス関数と Direct3D のランタイム バージョン 10 と 11 関数へのアクセスします。 これらの関数は、Direct3D ランタイムを通じて、オペレーティング システムを実装するユーザー モード Direct3D ディスプレイ ドライバー インターフェイスの一部です。

## <a name="direct3d-runtime-kernel-services-accessing-functions"></a>Direct3D カーネル サービスのランタイム関数へのアクセス

**Microsoft Direct3D のバージョン 9**ランタイムへのポインターを提供する*アダプター固有*のメンバーを使用してコールバック関数、 [D3DDDI_ADAPTERCALLBACKS](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3dumddi/ns-d3dumddi-_d3dddi_adaptercallbacks)構造体ユーザー モードへの呼び出しでは、ディスプレイ ドライバーの[OpenAdapter](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3dumddi/nc-d3dumddi-pfnd3dddi_openadapter)関数。 ランタイムに表示するポインターを提供する*デバイス固有*のメンバーを使用してコールバック関数、 [D3DDDI_DEVICECALLBACKS](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3dumddi/ns-d3dumddi-_d3dddi_devicecallbacks)ユーザー モードのディスプレイ ドライバーの呼び出しで構造[CreateDevice](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3dumddi/nc-d3dumddi-pfnd3dddi_createdevice)関数。 

**Microsoft バージョン direct3d10**またはそれ以降の実行時のメンバーを使用してアダプターに固有のコールバック関数へのポインターを指定する、 **D3DDDI_ADAPTERCALLBACKS**ユーザー モードへの呼び出しで構造体ディスプレイ ドライバーの[OpenAdapter10](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3d10umddi/nc-d3d10umddi-pfnd3d10ddi_openadapter)または OpenAdapter10_2 関数。 表示のメンバーを使用してデバイスに固有のコールバック関数へのポインターを提供する、ランタイム、 **D3DDDI_DEVICECALLBACKS**ユーザー モードのディスプレイ ドライバーの呼び出しで構造[CreateDevice(D3D10)](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3d10umddi/nc-d3d10umddi-pfnd3d10ddi_createdevice)関数。
...
このセクションでは、Direct3D ランタイム カーネル サービスによって使用される構造を説明します。 機能にアクセスします。 ユーザー モードのディスプレイ ドライバーは、ランタイム関数のパラメーターにこれらの構造体にポインターを渡します。
...
## <a name="direct3d-runtime-version-10-and-later-core-callback-functions"></a>Direct3D のランタイム バージョン 10 以降のコアのコールバック関数します。

このセクションでは、Microsoft direct3d10 およびそれ以降のランタイムは、ユーザー モードのディスプレイ ドライバーを提供する中心的なコールバック機能について説明します。 ランタイムのメンバーを使用してコア コールバック関数へのポインターを提供する、 [D3D10DDI_CORELAYER_DEVICECALLBACKS](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3d10umddi/ns-d3d10umddi-d3d10ddi_corelayer_devicecallbacks)ユーザー モードのディスプレイ ドライバーの呼び出しで構造[CreateDevice(D3D10)](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3d10umddi/nc-d3d10umddi-pfnd3d10ddi_createdevice)関数。

### <a name="direct3d-runtime-version-10-control-callback-functions"></a>Direct3D のランタイム バージョン 10 コントロールのコールバック関数

次には、ユーザー モードのディスプレイ ドライバーを Microsoft direct3d10 およびそれ以降のランタイムを提供するコントロールのコールバック関数の一覧を示します。
...
### <a name="direct3d-runtime-version-10-state-refresh-callback-functions"></a>Direct3D のランタイム バージョン 10 状態更新コールバック関数

このセクションでは、リファレンスのページには、ユーザー モードのディスプレイ ドライバーを Microsoft direct3d10 ランタイムを提供する状態更新コールバック関数がについて説明します。 

Direct3d10 ランタイムがアプリケーションの現在のバインド済み状態オブジェクトをキャッシュするため、ランタイムも少ないオーバーヘッドでユーザー モードのディスプレイ ドライバーの現在のバインド済み状態オブジェクトをキャッシュします。 各呼び出しの状態更新コールバック関数の場合、ランタイムは、ドライバーでは、呼び出し元のコードに戻る前に、同じ実行スレッドでドライバーの状態の関数に対応する呼び出しを direct3d10 をユーザー モードのディスプレイ ドライバーになります。 パフォーマンスを向上させるのには、パラメーター検証を実行して、状態の更新のコールバック関数。 

状態の更新のコールバック関数は、ステートレスのドライバーまたはコマンド バッファー プリアンブル データの蓄積を開発している場合に便利です。 状態の更新のコールバック関数では、ユーザー モードのディスプレイ ドライバー ハイ ウォーターマーク direct3d10 ランタイムは保持がメリットを享受することもできます。 ハイ ウォーターマークは、NULL 以外の場合; である可能性があります、最大スロット インデックスを示しますしたがって、ハイ ウォーターマークは、このようなスロットにわたってトラバーサルを向上します。
...
## <a name="direct3d-runtime-version-10-kernel-services-accessing-functions"></a>Direct3D カーネル サービスのランタイム バージョン 10 関数へのアクセス

このセクションでは、カーネル サービスを説明します。 Microsoft direct3d10 ランタイムの DirectX Graphics Infrastructure (DXGI) コンポーネントをユーザー モードのディスプレイ ドライバーを提供する機能にアクセスします。 カーネル サービスへのポインターを提供する DXGI のメンバー関数へのアクセス、 [DXGI_DDI_BASE_CALLBACKS](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/dxgiddi/ns-dxgiddi-dxgi_ddi_base_callbacks)ユーザー モードのディスプレイ ドライバーの呼び出しで構造[CreateDevice(D3D10)](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3d10umddi/nc-d3d10umddi-pfnd3d10ddi_createdevice)関数.

## <a name="direct3d-runtime-version-11-functions"></a>Direct3D のランタイム バージョン 11 関数

このセクションでは、Microsoft direct3d11 およびそれ以降のランタイムは、ユーザー モードのディスプレイ ドライバーを提供する中心的なコールバック機能について説明します。 ランタイムのメンバーを使用してコア コールバック関数へのポインターを提供する、 [D3D11DDI_CORELAYER_DEVICECALLBACKS](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/d3d10umddi/ns-d3d10umddi-d3d11ddi_corelayer_devicecallbacks)ユーザー モードのディスプレイ ドライバーの CreateDevice(D3D10) 関数の呼び出しで構造体。


### <a name="direct3d-runtime-version-11-control-callback-functions"></a>Direct3D のランタイム バージョン 11 コントロールのコールバック関数

このセクションでは、ユーザー モードのディスプレイ ドライバーを Microsoft direct3d11 およびそれ以降のランタイムを提供するコントロールのコールバック関数について説明します。
...
### <a name="direct3d-runtime-version-11-state-refresh-callback-functions"></a>Direct3D ランタイムのバージョン 11 の状態更新コールバック関数

このセクションでは、ユーザー モードのディスプレイ ドライバーを Microsoft バージョン direct3d11 およびそれ以降のランタイムを提供する状態更新コールバック関数について説明します。 

Direct3d11 のランタイムがアプリケーションの現在のバインド済み状態オブジェクトをキャッシュするため、ランタイムも少ないオーバーヘッドでユーザー モードのディスプレイ ドライバーの現在のバインド済み状態オブジェクトをキャッシュします。 各呼び出しの状態更新コールバック関数の場合、ランタイムは、ドライバーでは、呼び出し元のコードに戻る前に、同じ実行スレッドでドライバーの状態の関数に対応する呼び出しを direct3d11 をユーザー モードのディスプレイ ドライバーになります。 パフォーマンスを向上させるのには、パラメーター検証を実行して、状態の更新のコールバック関数。 

状態の更新のコールバック関数は、ステートレスのドライバーの開発やコマンド バッファー プリアンブル データを構築しようとする場合に便利です。 状態の更新のコールバック関数では、ユーザー モードのディスプレイ ドライバー高透かし、direct3d11 ランタイムを保持するメリットを享受することもできます。 ハイ ウォーターマークは、NULL 以外の場合; である可能性があります、最大スロット インデックスを示しますしたがって、ハイ ウォーターマークは、このようなスロットにわたってトラバーサルを向上します。
...
## <a name="direct3d-runtime-version-12-and-later-functions"></a>Direct3D のランタイム バージョン 12 以降関数します。

このセクションでは、リファレンスのページには、Microsoft direct3d12 およびそれ以降のランタイムは、ユーザー モードのディスプレイ ドライバーを提供する中心的なコールバック機能について説明します。 
...
[Multiplane オーバーレイのサポート](multiplane-overlay-support.md)

[ユーザー モードのディスプレイ ドライバーによって実装された Direct3D 関数](direct3d-functions-implemented-by-user-mode.md)
