...
description: OpenGL インストール可能なクライアントドライバーにカーネルモードのサポートを提供する
ms.assetid: 1871594a-ca4d-4a3c-bf12-bbf80fecefe9
keywords:
- OpenGL ICD WDK ディスプレイ
- カーネルモード OpenGL ICD WDK ディスプレイ
- の場合 WDK の表示
...
ms.locfileid: "72829671"
---
# <a name="kernel-mode-support-to-the-opengl-installable-client-driver"></a>OpenGL のインストール可能なクライアント ドライバーに対するカーネル モードのサポート


OpenGL のインストール可能なクライアントドライバー (ICD) では、 [Direct3D ユーザーモードディスプレイドライバー](initializing-communication-with-the-direct3d-user-mode-display-driver.md)と同じレベルのカーネルモードサービスを呼び出すことができます。 ただし、次のコード例に[示すように、OPENGL ICD](https://docs.microsoft.com/windows-hardware/drivers/ddi/index)は、 [**D3DDDIARG\_openadapter**](https://docs.microsoft.com/windows-hardware/drivers/ddi/d3dumddi/ns-d3dumddi-_d3dddiarg_openadapter)構造体の**Padaptercallbacks**メンバーおよび[**D3DDDIARG\_CREATEDEVICE**](https://docs.microsoft.com/windows-hardware/drivers/ddi/d3dumddi/ns-d3dumddi-_d3dddiarg_createdevice)構造体の**Padaptercallbacks**メンバーを介して提供される、Microsoft Direct3D ランタイムなどのコールバック関数を介してカーネルモードサービスへのアクセスを取得するのではなく、Gdi32 を読み込む必要があります。 このコード[では、OpenGL で Windows 8 の機能強化](supporting-opengl-enhancements.md)が実装されていません。
