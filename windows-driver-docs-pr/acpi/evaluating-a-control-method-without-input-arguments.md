...
入力引数を受け取らない制御メソッドを同期的に評価するために、デバイスのドライバーは、デバイスに対する acpi [ **\_eval\_メソッド**](https://docs.microsoft.com/windows-hardware/drivers/ddi/acpiioct/ni-acpiioct-ioctl_acpi_eval_method)要求または[**ioctl\_acpi\_eval\_メソッド\_EX**](https://docs.microsoft.com/windows-hardware/drivers/ddi/acpiioct/ni-acpiioct-ioctl_acpi_eval_method_ex)要求を\_します。 これらの要求の両方を使用するための一般的な手順については、「 [ACPI 制御メソッドを同期的に評価](evaluating-acpi-control-methods-synchronously.md)する」を参照してください。 この2つの要求を使用する場合の具体的な違いは次のとおりです。
...
-   コントロールメソッドがデバイスの ACPI 名前空間の子オブジェクトであるが、デバイスの直接の子オブジェクトではない場合、ドライバーは IOCTL\_ACPI\_EVAL\_メソッド\_EX 要求に送信し、 [**acpi\_eval\_INPUT\_ex**](https://docs.microsoft.com/windows-hardware/drivers/ddi/acpiioct/ns-acpiioct-_acpi_eval_input_buffer_v1_ex)構造体を提供します。\_
...
*Getabcdata*はまず、ACPI\_EVAL\_入力\_バッファー構造*inputBuffer*に割り当て、 **methodnameasulong**メンバーをコントロールメソッドの名前に設定し、**署名**メンバーを ACPI\_EVAL\_input\_BUFFER\_signature に設定します。
...
この手順はサンプルコードには含まれていませんが、ドライバーは、出力データを処理した後に[**IoCompleteRequest**](https://docs.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocompleterequest)を呼び出して、保留中の IOCTL\_ACPI\_EVAL\_method 要求または IOCTL\_ACPI\_EVAL\_method 要求を完了してから、コントロールメソッドを評価します。
