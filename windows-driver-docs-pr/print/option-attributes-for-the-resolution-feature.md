...
ms.locfileid: "56549271"
---
# <a name="option-attributes-for-the-resolution-feature"></a>解決策機能のオプション属性
...
<td><p>X と y を表す数値の値のペアは、プリンターの値&#39;s 解像度、ドット/インチにします。</p></td>
...
<td><p>(省略可能)。 指定しない場合、既定値は 0 です。 この属性は、関連する場合にのみ、 <em> <strong>StripBlanks</strong> ENCLOSED を指定します。 参照してください<a href="raster-data-emission-attributes.md" data-raw-source="[Raster Data Emission Attributes](raster-data-emission-attributes.md)">ラスター データ出力属性</a>します。</p></td>
</tr>
<tr class="odd">
...
<td><p>(省略可能)。 指定しない場合、既定値は 1 です。 インター レース、一連のすべてのスキャン ラインを印刷する、スキャン ラインの印刷ヘッドの複数のパスを必要とするかどうか、プリンターを実行しますが必要です。</p></td>
</tr>
<tr class="even">
<td><p></em><strong>PinsPerPhysPass</strong></p></td>
<td><p>印刷ヘッドが、ページ間で移動すると、スキャン ラインの数を表す数値が印刷されます。 1 つ、または 8 の倍数である必要があります。</p></td>
<td><p>(省略可能)。 指定しない場合、既定値は 1 です。</p>
...
<td><p>ペアまたは x を表す数値とプリンターの y 値&#39;s テキスト解像度のドット/インチにします。</p></td>
