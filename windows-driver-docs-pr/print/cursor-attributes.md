...
ms.locfileid: "56528187"
---
# <a name="cursor-attributes"></a>カーソルの属性
...
<td><p>(省略可能)。 指定されていない場合は、既定値に制限はありません。 たとえば、LIST(X_PORTRAIT) では、縦向きの x 方向の移動は許可されていませんことを示します。</p></td>
</tr>
<tr class="odd">
<td><p><strong><em>CursorXAfterCR</strong></p></td>
<td><p></p>
いずれか:AT_PRINTABLE_X_ORIGIN AT_CURSOR_X_ORIGIN
<p>カーソルを示します&#39;復帰後 s の x 位置。</p></td>
<td><p>(省略可能)。 既定値は、これは、物理的な AT_CURSOR_X_ORIGIN で指定しない場合、位置は 0。</p></td>
</tr>
<tr class="even">
...
<td><p>(省略可能)。 既定値は、1 マスターの単位です。</p></td>
</tr>
<tr class="even">
<td><p><strong></em>MaxLineSpacing</strong></p></td>
<td><p>行の最大間隔を y マスター単位で表す数値。</p></td>
<td><p>(省略可能)。 指定しない場合、Unidrv は、最大値がないと仮定します。</p></td>
</tr>
<tr class="odd">
...
<td><p>(省略可能)。 指定されていない場合、既定値は<strong>TRUE</strong>します。</p>
...
<td><p>(省略可能)。 既定値が 0 の場合に指定しない場合、意味<strong>CmdXMoveAbsolute</strong>常に使用する必要があります。 次の 3 つのすべての x 移動コマンドが指定されている場合にのみ適用されます。</p></td>
</tr>
<tr class="odd">
...
<td><p>(省略可能)。 指定しない場合、属性がないと仮定します。</p></td>
</tr>
<tr class="odd">
...
<td><p>(省略可能)。 既定値が 0 の場合に指定しない場合、意味<strong>CmdYMoveAbsolute</strong>常に使用する必要があります。 次の 3 つのすべての y 移動コマンドが指定されている場合にのみ適用されます。</p></td>
</tr>
<tr class="even">
