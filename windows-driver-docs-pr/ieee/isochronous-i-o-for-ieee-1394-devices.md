---
title: IEEE 1394 のデバイスの I/O はアイソクロナス
description: IEEE 1394 のデバイスの I/O はアイソクロナス
ms.assetid: fc544776-af45-40e2-9699-7dcc50275d1e
keywords:
- IEEE 1394 WDK バス、アイソクロナス I/O
- 1394 WDK バス、アイソクロナス I/O
- I/O WDK の IEEE 1394 バス
- I/O 要求パケット WDK IEEE 1394 バス
- Irp WDK IEEE 1394 バス
- アイソクロナス I/O WDK の IEEE 1394 バス
- WDK の IEEE 1394 の帯域幅のバスの保証
- WDK の IEEE 1394 のバスの帯域幅
- アイソクロナス I/O WDK の IEEE 1394 バス、アイソクロナス I/O の概要
- WDK の IEEE 1394 のデータを転送します。
ms.date: 04/20/2017
ms.localizationpriority: medium
ms.openlocfilehash: 1f51c6ed708badb260aa34d715aa89ccc11df8a3
ms.sourcegitcommit: a33b7978e22d5bb9f65ca7056f955319049a2e4c
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/31/2019
ms.locfileid: "56560737"
---
# <a name="isochronous-io-for-ieee-1394-devices"></a>IEEE 1394 のデバイスの I/O はアイソクロナス





デジタル カメラなどのリアルタイムのマルチ メディア デバイスは、大量の帯域幅、安定でデータを送信する必要がありますが、配信の保証を必要としません。 (たとえば、デジタル カメラからドロップされたフレームには、信号の品質が低下するが、その意味を破棄が)。IEEE 1394 は、このようなデバイスでは、*アイソクロナス*転送では、保証された帯域幅を提供しますが、配送を保証しません。

このセクションの内容:

[IEEE 1394 デバイス Isochronous 転送の設定](https://msdn.microsoft.com/library/windows/hardware/ff538108)

[IEEE 1394 デバイス バッファリング アイソクロナス DMA 転送](https://msdn.microsoft.com/library/windows/hardware/ff537014)

[IEEE 1394 デバイス アイソクロナス リッスン オプション](https://msdn.microsoft.com/library/windows/hardware/ff537377)

[IEEE 1394 デバイス アイソクロナス トーク オプション](https://msdn.microsoft.com/library/windows/hardware/ff537380)

[IEEE 1394 デバイス アイソクロナス同期オプション](https://msdn.microsoft.com/library/windows/hardware/ff537379)

[Isochronous データ転送を完了します。](https://msdn.microsoft.com/library/windows/hardware/ff537058)

 

 




