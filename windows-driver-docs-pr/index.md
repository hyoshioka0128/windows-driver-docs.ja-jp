---
title: Windows Driver Kit のドキュメント
description: このセクションでは、WDK とサンプルの入手方法と Windows ドライバーの構築方法について説明します。
services: wdk
layout: LandingPage
ms.assetid: 5502AAF9-2400-4338-A646-C746B29F9A44
ms.date: 04/20/2017
ms.topic: landing-page
ms.prod: windows-hardware
ms.technology: windows-devices
ms.openlocfilehash: 18925bb8f93296690f3d8a05ccdb0b52058e3b42
ms.sourcegitcommit: 56599ec634b3a731f2d13dff686be3b7b95390e4
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 03/25/2019
ms.locfileid: "58419550"
---
# <a name="windows-driver-kit-documentation"></a>Windows Driver Kit のドキュメント

Windows Driver Kit (WDK) 10 は Microsoft Visual Studio 2017 および [Debugging Tools for Windows](/windows-hardware/drivers/debugger/index) と統合されています。この統合環境には、ドライバーの開発、構築、パッケージ化、展開、テスト、およびデバッグに必要なツールが用意されています。 統合環境内で、多くの基本的な認定テストを実行できます。 Windows Driver Kit (WDK) には、[Windows Driver Frameworks](https://docs.microsoft.com/windows-hardware/drivers/wdf/)、ユニバーサル シリアル バス (USB)、印刷、ネットワーク、ファイル システム フィルターを含む、いくつかのテクノロジとドライバー モデルのテンプレートが含まれています。

<ul class="cardsY panelContent featuredContent">
    <li>
        <a href="/windows-hardware/drivers/develop/getting-started-with-universal-drivers">
            <div class="cardSize">
                <div class="cardPadding">
                    <div class="card">
                        <div class="cardImageOuter">
                            <div class="cardImage"> 
                                <img src="https://docs.microsoft.com/media/common/i_get-started.svg" alt="A button that links to the topic: Getting Started with the Windows Driver Kit. The button has a picture of a rocket taking off." />
                            </div>
                        </div>
                            <div class="cardText">
                                <h3><b>ユニバーサル Windows ドライバーの概要</b></h3>
                            </div>
                    </div>
            </div>
        </a>
    </li>
    <li>
        <a href="/windows-hardware/drivers/ddi/">
            <div class="cardSize">
                <div class="cardPadding">
                    <div class="card">
                        <div class="cardImageOuter">
                            <div class="cardImage"> 
                                <img src="https://docs.microsoft.com/media/common/i_api-reference.svg" alt="Icon for WDK DDI reference topics: a circle with a monitor and a power button" />
                            </div>
                        </div>
                        <div class="cardText">
                            <h3><b>WDK DDI リファレンス</b></h3>
                        </div>
                    </div>
                </div>
            </div>
        </a>
    </li>
</ul>

<ul class="cardsF panelContent">
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_whats-new.svg" alt="Icon for What's New in Driver Kit docs: a bullhorn">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>新着情報</h3>
                        <p><a href="/windows-hardware/drivers/what-s-new-in-driver-development">新しい API と新機能</a></p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_download-install.svg" alt="Icon for Download and Install: a monitor showing a blue gear near a downward pointing arrow">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>ダウンロードとインストール</h3>
                        <p>
                            <a href="https://go.microsoft.com/fwlink/p/?LinkId=698539">Visual Studio 2017</a>
                        </p>
                        <p>
                            <a href="https://developer.microsoft.com/windows/hardware/windows-driver-kit">Windows Driver Kit (WDK)</a>
                        </p>                        
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_code-samples.svg" alt="Icon for Code Samples: curly brackets around three blue rectangles, stacked on each other">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>コード サンプル</h3>
                        <p>
                            <a href="https://github.com/Microsoft/Windows-driver-samples">ユニバーサル Windows ドライバーのサンプル</a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_guidelines.svg" alt="Icon for Programming Guides: A book, Two pages with unnumbered lists. Above one page, a check mark; above the other, an X">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>プログラミング ガイド</h3>
                        <p>
                            <a href="/windows-hardware/drivers/gettingstarted/choosing-a-driver-model">ドライバー モデルの選択</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/gettingstarted/writing-your-first-driver">初めてのドライバーの作成</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/debugger/">Windows デバッグ ツール</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/gettingstarted/concepts-and-knowledge-for-all-driver-developers">すべてのドライバー開発者のための概念</a>
                        </p>                        
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_build.svg" alt="Icon for Testing and Deploying Drivers: a crane, lifting a package">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>ドライバーのテストと展開</h3>
                        <p>
                            <a href="/windows-hardware/drivers/develop/building-a-driver">WDK を使ったドライバーのビルド</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/gettingstarted/provision-a-target-computer-wdk-8-1">ドライバーの展開およびテストのためのコンピューターのプロビジョニング (WDK 10)</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/develop/testing-a-driver">ドライバーのテスト</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/install/">デバイスとドライバーのインストール</a>
                        </p>                      
                    </div>
                </div>
            </div>
        </div>
    </li><br>    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_upgrade.svg" alt="Icon for Submission: a circle filled white over blue, with an arrow pointing upward">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>申請</h3>
                        <p>
                            <a href="/windows-hardware/drivers/dashboard/">Windows ハードウェア デベロッパー センター ダッシュボード</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/develop/distributing-a-driver-package-win8">ドライバー パッケージの配布</a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_architecture.svg" alt="Icon for Driver Models and Technologies: a flow chart with a blue diamond linking to two white rectangles and one white circle">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>ドライバーのモデルとテクノロジ</h3>
                        <p>
                            <a href="/windows-hardware/drivers/kernel/">カーネルモード ドライバー アーキテクチャ設計ガイド</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/wdf/">Windows Driver Frameworks</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/device-and-driver-technologies">デバイス テクノロジとドライバー テクノロジ</a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </li>
    <li>
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="https://docs.microsoft.com/media/common/i_link.svg" alt="Icon for Related Links: two links from a chain">
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>関連リンク</h3>
                        <p>
                            <a href="/windows-hardware/drivers/bringup/">起動ガイド</a>
                        </p>
                        <p>
                            <a href="/windows-hardware/drivers/devapps/">Windows ストア デバイス アプリ</a>
                        </p>
                        <p>
                            <a href="https://docs.microsoft.com/windows/uwp/">UWP アプリ</a>
                        </p>
                        <p>
                            <a href="https://developer.microsoft.com/windows/desktop">デスクトップ アプリケーション</a>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </li>
</ul>

<p><b>WDK 10 の実行時の要件</b></p>

WDK には Visual Studio が必要です。 詳細については、「[Visual Studio 2017 のシステム要件](https://www.visualstudio.com/productinfo/vs2017-system-requirements-vs)」を参照してください。

EWDK にはさらに .NET 4.6.1 が必要です。 .NET の要件については、「[.NET Framework のシステム要件](https://docs.microsoft.com/dotnet/framework/get-started/system-requirements)」を参照してください。 

WDK Insider Preview と EWDK Insider Preview を使用すると、次のオペレーティング システムで動作するドライバーを開発できます。

|  クライアントの OS  |         サーバーの OS          |
|-------------|----------------------------|
| Windows 10  |    Windows Server 2016     |
| Windows 8.1 |   Windows Server 2012 R2   |
|  Windows 8  |    Windows Server 2012     |
|  Windows 7  | Windows Server 2008 R2 SP1 |
