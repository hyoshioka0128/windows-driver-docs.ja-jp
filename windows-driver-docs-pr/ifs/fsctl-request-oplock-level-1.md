...
description: FSCTL\_要求\_OPLOCK\_レベル\_1 コントロールコードは、ファイルにレベル1の便宜ロック (oplock) を要求します。
ms.assetid: 67ffb92c-d638-46fc-9afb-9f1322db2f3b
keywords:
- FSCTL_REQUEST_OPLOCK_LEVEL_1 制御コードのインストール可能なファイルシステムドライバー
topic_type:
- apiref
api_name:
...
ms.locfileid: "72841259"
---
# <a name="fsctl_request_oplock_level_1-control-code"></a>FSCTL\_要求\_OPLOCK\_レベル\_1 コントロールコード


**FSCTL\_要求\_OPLOCK\_レベル\_1**コントロールコードは、ファイルにレベル1の便宜ロック (oplock) を要求します。
...
便宜的ロックの詳細および**FSCTL\_要求\_OPLOCK\_レベル\_1**コントロールコードについては、Microsoft Windows SDK のドキュメントを参照してください。
...
[**FltOplockFsctrl**](https://docs.microsoft.com/windows-hardware/drivers/ddi/fltkernel/nf-fltkernel-fltoplockfsctrl)のみです。 IRP\_MJ\_ファイル\_システム\_CONTROL FSCTL request のコールバックデータ ([**FLT\_callback\_data**](https://docs.microsoft.com/windows-hardware/drivers/ddi/fltkernel/ns-fltkernel-_flt_callback_data)) 構造。 操作の*Fscontrolcode*パラメーターは、FSCTL\_要求\_OPLOCK\_レベル\_1 である必要があります。

<a href="" id="irp"></a>*Irp*  
[**FsRtlOplockFsctrl**](https://docs.microsoft.com/windows-hardware/drivers/ddi/ntifs/nf-ntifs-_fsrtl_advanced_fcb_header-fsrtloplockfsctrl)のみです。 Irp\_MJ\_ファイル\_システム\_CONTROL FSCTL 要求の IRP。 操作の*Fscontrolcode*パラメーターは、FSCTL\_要求\_OPLOCK\_レベル\_1 である必要があります。
...
<td align="left"><p>IRP は、FSCTL_REQUEST_OPLOCK_LEVEL_1 操作が完了する前に取り消されました。 これはエラーコードです。</p></td>
</tr>
<tr class="odd">
