...
- **[作成後] で、ECP が確認された場合は、ECP リダイレクトフラグを確認します。** フラグは、開いているがパッケージレイヤーまたはスクラッチルート (新規または変更されたファイル) からサービスを処理したかどうかを示します。 フラグは、パッケージレイヤーが登録されているかどうか、およびリモートであるかどうかも示します。
  ...
  - *登録されたレイヤーからサービスが提供される場合*、AV はファイルのスキャンをスキップします。 これはリダイレクトフラグによって示されます: `WCIFS_REDIRECTION_FLAGS_CREATE_SERVICED_FROM_LAYER &&  WCIFS_REDIRECTION_FLAGS_CREATE_SERVICED_FROM_REGISTERED_LAYER`
