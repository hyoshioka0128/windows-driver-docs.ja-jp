...
-   V @ no__t_0_ NET\_ルート--net ルートのビュー (仮想 netroots とも呼ばれます)。
...
<td align="left"><p>FCB をポイントする SRV_OPEN エントリの数と、いくつかの動的な値。</p></td>
</tr>
<tr class="odd">
...
ネットワークミニリダイレクターと RDBSS の間のインターフェイスで使用される2つの重要な抽象化は、SRV\_呼び出しと NET\_ルート構造です。 SRV\_呼び出し構造は、接続が確立されたサーバーに関連付けられたコンテキストに対応します。また、NET\_ルート構造は、サーバー上の共有に対応します (これは名前空間の一部としても表示される可能性があります)。(ネットワークミニリダイレクターによって要求されている)。
