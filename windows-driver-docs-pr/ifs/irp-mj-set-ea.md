...
keywords:
- インストール可能なファイルシステムドライバーの IRP_MJ_SET_EA
topic_type:
- apiref
api_name:
- IRP_MJ_SET_EA
...
*Irpsp-&gt;FileObject*パラメーターには、関連する**fileobject**フィールドへのポインターが含まれています。これは、ファイル\_オブジェクト構造体でもあります。 ファイル\_オブジェクト構造の\_の対象となるフィールドは、IRP\_MJ の処理中は無効**であり、** EA\_設定されているため、使用できません。
