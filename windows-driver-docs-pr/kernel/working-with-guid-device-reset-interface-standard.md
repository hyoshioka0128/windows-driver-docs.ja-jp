...
Windows 10 以降、これらのレジストリ エントリの下、`HKLM\SYSTEM\CurrentControlSet\Control\Pnp`キーのリセット操作を構成します。 
...
> [!NOTE]
>  GUID_DEVICE_RESET_INTERFACE_STANDARD インターフェイスでは、Windows 10 以降使用できます。
...
1. Asl.exe などの ASL コンパイラを使用して、AML テスト ASL ファイルをコンパイルします。 実行可能ファイルには、Windows Driver Kit (WDK) で含まれています。 
