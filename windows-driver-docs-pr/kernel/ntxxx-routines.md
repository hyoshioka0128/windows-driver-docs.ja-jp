...
ms.locfileid: "67393416"
---
# <a name="ntxxx-routines"></a>NtXxx ルーチン


このセクションについて説明します、 **Nt * Xxx*** バージョンの Windows のネイティブ システム サービス ルーチン。 ほとんどのネイティブ システム サービス ルーチンがある 2 つのバージョン、1 つに名前が、プレフィックスで始まる**Nt**; その他のバージョンが、プレフィックスで始まる名前**Zw**します。 たとえば、呼び出し[ **NtCreateFile** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntifs/nf-ntifs-ntcreatefile)と[ **ZwCreateFile** ](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/ntifs/nf-ntifs-ntcreatefile)同様の操作を実行し、同じサービスが、実際には、カーネル モード システム ルーチンです。

カーネル モード ドライバーからの呼び出し、 **Nt * Xxx*** と**Zw * Xxx*** のバージョンの Windows のネイティブ システム サービス ルーチンは、処理と、入力パラメーターの解釈で異なった動作できます。 間のリレーションシップの詳細については、 **Nt * Xxx*** と**Zw * Xxx***、ルーチンのバージョンを参照してください[を使用して Nt と Zw バージョンのネイティブ システム サービス ルーチン](using-nt-and-zw-versions-of-the-native-system-services-routines.md).

次の表にまとめたものです、 **Nt * Xxx*** と**Zw * Xxx*** ルーチンのバージョン。
