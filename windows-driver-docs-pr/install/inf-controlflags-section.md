...
-1 ユニバーサルまたはモバイルのドライバーパッケージをビルドしている場合、このセクションは有効ではない**ことに注意**してください @no__t。 「[ユニバーサル INF ファイルの使用」を](using-a-universal-inf-file.md)参照してください。
...
すべてを削除します (\* が指定されている場合)。または特定のユーザーインターフェイスから指定されたデバイスの一覧が表示され、ユーザーはインストールする特定のデバイスを選択することが想定されます。
...
システム定義の**nt**、 **.ntx86**、 **.ntia64**、 **.ntamd64**、 **.ntarm**、および **.ntarm64**の各拡張機能の使用方法の詳細については、「[複数のプラットフォームおよびオペレーティングシステムの INF ファイルの作成」を参照してください](creating-inf-files-for-multiple-platforms-and-operating-systems.md)。
...
すべて ( **\*** を指定した場合)、または指定したデバイスの一覧に、デバイスの[ドライバーパッケージ](driver-packages.md)を通じてインストールされたソフトウェアよりも追加のソフトウェアが必要な場合があることを指定します。 たとえば、 **Requestadditionalsoftware**エントリを使用すると、ドライバーパッケージに含まれていなかった新しいデバイスまたは更新されたソフトウェアをインストールできます。

**メモ** **@No__t-2**が指定されていない場合は、 **requestadditionalsoftware**エントリによって指定された各デバイスを、 [**INF モデルセクション**](inf-models-section.md)内で定義する必要があります。
...
2.  デバイス固有のソフトウェアに対して独立系ハードウェアベンダー (IHV) によって提供されるソリューションがある場合、ソリューションはコンピューターにダウンロードされます。
    ...
    **注**  The をダウンロードしてもソフトウェア自体はインストールされません。
...
通常、 **Controlflags**セクションには、[製造元別の[**INF モデル] セクション**](inf-models-section.md)に一覧表示されているデバイスを識別する1つ以上の**ExcludeFromSelect**エントリがありますが、手動でオプションとしてエンドユーザーに表示することはできません。レーション.
...
**注**@no__t-その後、WHQL では、INF ファイルに**interactiveinstall**エントリが含まれるデバイスに Windows ロゴが付与されない場合があります。
