...
| INF * \[の追加-インターフェイスセクション\] *セクション                        | デバイスインターフェイスのレジストリキー                                                    |
...
**Note**  この値は、指定された値エントリの既定の型であることに注意してください。そのため、この型の値のエントリに対して動作する、 *add-* *root =* 行の flags 値を省略できます。
...
**Note**  フラグ値の下位ワードの下位ビットは、文字データとバイナリデータを区別することに注意してください。
...
**DeviceCharacteristics** Hkr **AddReg**エントリは、デバイスの特性を指定します。 *特性*値は、 *Ntddk および*で定義されている1つ*以上の FILE_* \*ファイル特性値に対してまたはを使用した結果の数値です。
...
**重要**  DLL と**extensionpropsheet pageproc** callback 関数の名前は両方とも引用符 ("") で囲む必要があります。
