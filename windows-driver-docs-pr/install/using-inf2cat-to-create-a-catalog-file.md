...
カタログの名前のファイルを[ **Inf2Cat** ](https://docs.microsoft.com/windows-hardware/drivers/devtest/inf2cat)が生成されますが、CatalogFile ディレクティブによって指定されます。 1 つ以上のこれらのディレクティブが内で宣言された、 [ **INF バージョン セクション**](inf-version-section.md)の INF ファイル。 INF**バージョン**のセクション、 *toastpkg.inf*ファイルを次に示します。
...
1. [ **INF バージョン セクション**](inf-version-section.md)ドライバー パッケージをサポートする Windows バージョンごとに 1 つずつ、3 つの別のカタログ ファイルを宣言します。 ときに[ **Inf2Cat** ](https://docs.microsoft.com/windows-hardware/drivers/devtest/inf2cat)がで指定された各 Windows バージョンのカタログ ファイルを作成、実行、 **/os**オプション。
...
2. [ **DriverVer ディレクティブ**](inf-driverver-directive.md)古いタイムスタンプおよびバージョンを INF のバージョンのセクションを宣言します。
   ...
   使用する前に[ **Inf2Cat**](https://docs.microsoft.com/windows-hardware/drivers/devtest/inf2cat)、する必要があること確認する INF ファイルの**DriverVer**ディレクティブが現在の時刻スタンプとバージョンの値。 これは必要、[ドライバー パッケージ](driver-packages.md)をインストールして、テスト コンピューター上のパッケージの以前にインストールされたバージョンが置き換えられます。

   使用することができます、 [Stampinf](https://docs.microsoft.com/windows-hardware/drivers/devtest/stampinf)で時刻のスタンプとバージョンの値を更新するためのツール、 **DriverVer**ディレクティブ。 たとえば、更新するため、 **DriverVer**ディレクティブで、 *toastpkg.inf*、次のコマンドを実行して<em>:</em>
...
ツールのコマンドライン引数の詳細については、次を参照してください。 [ **Inf2Cat**](https://docs.microsoft.com/windows-hardware/drivers/devtest/inf2cat)します。
