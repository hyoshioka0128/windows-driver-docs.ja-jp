...
早期起動ドライバーでは、監視し、各ブート開始ドライバーの入力として使用される構成データの検証をレジストリまたはブート ドライバーのコールバックを使用できます。 構成データは、Winload によって読み込まれ、使用可能なブート ドライバーの初期化時に、システム レジストリ ハイブに格納されます。

> [!NOTE]
> システム起動前に、ELAM のレジストリ ハイブへの変更は破棄されます。
> 結果として、ELAM ドライバーは、標準 Event Tracing for Windows (ETW)、レジストリに書き込むのではなく、ログ記録を使用してください。
