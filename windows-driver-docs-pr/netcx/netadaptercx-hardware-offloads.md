...
TCP/IP スタックまたはそれ以降のプロトコルドライバーがネットワークアダプターのアクティブな機能の変更を要求した場合、NetAdapterCx は、アダプターの初期化時に以前に登録されたクライアントドライバーの*EVT_NET_ADAPTER_OFFLOAD_SET_XxX*コールバック関数を呼び出します。 この関数では、システムは NETOFFLOAD obbject で更新された機能を提供します。この機能は、クライアントドライバーがクエリを実行してオフロード機能を更新するために使用できます。
